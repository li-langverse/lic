...
| 7d-b–e | Decorator elaborate + exploits | **G-dec**, **G-par** | **7d-b exit:** `@cpu`/`@parallel`/`@vectorized`/`@gpu` MIR proc tags + `check-mir-*-decorator.sh` gates; **7d-c exit:** `@parallel(disjoint=)` on `def` inherits to nested `parallel for` (policy witness, **G-par**); **7d-e exit:** `decorator_exploits/` CI green | language/decorators.md, fast-math guide |
...
- [ ] Phase 7d — Execution decorators — **partial (#22, #150):** **7d-b closed slice:** `@cpu`/`@parallel(disjoint=)`/`@vectorized`/`@gpu` MIR proc tags + `check-mir-*-decorator.sh` gates; `@vectorized` on `for` → `ArraySimdScope`; **def `@parallel(disjoint=)`** inherits to nested `parallel for` (policy, **G-par**); **open:** Lean **P-dec**, `@async` MIR tag, full LKIR `@gpu` lowering (**G-gpu**)
