...
2. **Worker processes** — homelab edge uses `LI_HTTPD_WORKERS=auto` (12 on blackpearl). Until the Li epoll spin wedge is fixed, **fewer workers are more stable** under parallel-18 burst:
   ...
   - **Blackpearl 2026-06-10 (lic `8b7ba2ef`, concurrent_streams=128):** `auto` cold-start → 18/18 single burst and 18/18×3 with 10s cooldown; **back-to-back** ×3 without restart → run1 18/18, run2 14/18 (000/truncation cascade). `workers=4` → 89% CPU wedge + truncation. `workers=2` → hung probe / 0–6/18. **Fixed** in follow-up: full `httpd_proxy_clear` + Li glue reset, upstream EPOLLHUP → `finish_err`, fresh upstream TCP per request (no pool reuse bleed), `g_active_proxy_streams` reconcile — back-to-back parallel-18 ×5 PASS on blackpearl.
