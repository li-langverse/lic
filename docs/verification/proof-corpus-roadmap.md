...
| **P-par** | `parallel for` disjointness | **Partial** — `_par*` → `disjoint_*_spec` + policy witnesses + MIR tag + `iteration_independent_tile_spec` compositional slice | Full memory-disjoint Lean specs (7d-c) |
