...
- **Remaining lib gaps (Phase 2):** `run_algo_registry_stub` only for non-registry / undocumented IDs (CFD/FEA/QM 205–216, 401–432 now tier-2); `li-sim-viz` compose-only; scene `native_pixels` stub; particles MD force accumulation still weak.
...
#### WP-SCI-03 — `run_algo_registry` real kernels (extends existing ID) — **LANDED** (2026-06-09)
...
- **Status:** CFD 205–210, FEA 211–216, QM 401–432 tier-2 dispatch landed; heat narrowed to 201–204; `vertical_cfd_lid_driven_cavity` / `vertical_fea_linear_elasticity` in `li-sim`.
