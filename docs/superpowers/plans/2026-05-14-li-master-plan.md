...
- [ ] Phase 7d — Execution decorators — **partial (#150, #387):** `@vectorized` on `for` → `ArraySimdScope`; **7d-b** `@vectorized` / `@parallel(disjoint=)` MIR proc tags; **def `@parallel(disjoint=)`** inherits to nested `parallel for` (policy); **closed slice:** `Li.Discharge.disjoint_*_spec` + `parallel_disjoint_lean_opaque_gap.sh`; **open:** iteration-independence Lean specs, tier-2 `@cpu` `@parallel` `@vectorized` on `def`
