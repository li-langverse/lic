...
- [ ] Phase 7d — Execution decorators — **partial (#22 7d-b/c):** proc `@parallel(disjoint=)` → `mir_parallel_disjoint`; proc `@vectorized` → `ArraySimdScope` (`mir_vectorized_def_scope`); `@vectorized` on `for` scoped (#150); **open:** **7d-e** macro expansion, Lean **G-par** proofs (**#387**) — [G-dec plan](2026-06-07-ph-7d-g-dec-mir-elaboration.md)
