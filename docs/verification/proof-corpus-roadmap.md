...
| **P-par** | `parallel for` disjointness | **Partial** — `_par*` → `disjoint_*_spec` + policy witnesses + MIR tag + `iteration_independent_tile_spec` + `memory_disjoint_rows_spec` + `memory_disjoint_elems_spec` + `memory_disjoint_grid_rows_spec` + `array_elem_indices_disjoint` + `array_row_indices_disjoint` compositional slices | Full dependent array aliasing Lean specs (7d-c) |
