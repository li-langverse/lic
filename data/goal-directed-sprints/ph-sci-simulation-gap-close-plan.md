...
| **li-sim-scientific** | partial → tier-2 oracle | CPU + `@gpu` MD oracle smoke | Lib builds with open VC; smokes verify | **Real:** 4-particle LJ chain + 1D heat stencil + CFD cavity Jacobi + FEA bar oracles, `run_multi_physics_at_step`. **Placeholder:** `run_algo_registry_stub` (checksum 1.001) for QM (401–432) and non-science families | WP-SCI-03 partial (CFD/FEA tier-2 landed); QM rows + LAMMPS/GROMACS external oracle remain |
...
- **Status:** partial (2026-06-08) — CFD (205–210) + FEA (211–216) tier-2 oracles landed; heat narrowed to 201–204; QM (401–432) still stub 1.001.
