...
| **G-num** | Stub | **WP0-A:** planned entries/num-*.toml + proof-db/num/; Peano/order linkage via **G-math**; no discharge slice yet. **Planned slice (ode-r3, lic#35):** `proof-database/entries/num-ode-bdf.toml` (`N-ODE-BDF1-LINEAR`, `N-ODE-BDF2-LINEAR`, `N-ODE-NEWTON-IMPLICIT`) — axiomatic/target; CVODE oracle gate `ph-sci-ode-oracle-competitive-gates.sh` |
