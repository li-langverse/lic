...
1. **WP-SCI-03** — `run_algo_registry` real kernels (CFD/FEA/QM rows); extend `run_algo_registry_tier2.li`. **Partial (2026-06-08):** algo **104** (`md_oracle_external`) tier-2 MD checksum dispatch + `run_algo_registry_tier2.li` row.
2. **WP-PLAT-05** — LAMMPS/GROMACS external oracle column (unblocks SCI-03 MD rows). **Partial (2026-06-08):** `md_oracle.toml` driver, competitive JSON bench/gates, `registry.toml` `lammps`/`gromacs` watch rows (external_binary stubs).
