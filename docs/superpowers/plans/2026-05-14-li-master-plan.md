...
- [x] Phase 2h — Python-math operators `%`, `//`, `**` (`math_syntax` suite); **`for i in start..<end`** (`for_range_sum.li`); **`for i in range(n)`** const bound (`for_range_*.li`); dynamic `range()` deferred (**G-math-syn**)
