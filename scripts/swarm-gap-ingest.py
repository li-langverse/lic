...
def ingest_verticals_stubs(gaps_by_id: dict[str, dict]) -> int:
    vert = Path(os.environ.get("BENCHMARKS_COMPETITIVE", str(LANGVERSE / "benchmarks/competitive"))) / "verticals.toml"
    if not vert.is_file():
        vert = Path(LANGVERSE / "benchmarks/competitive/verticals.toml")
    if not vert.is_file():
        return 0
