...
- **Environment**: WSL2 Docker on Windows workstation
...
- **li-httpd** image: `proxy-repro-proxy:latest` (lic `feat/dynamic-httpd-routes`, commit `e25c8b815` area).
- Field proxies: **100%** on all workloads including GitLab parallel-18 gate.
- **li-httpd** passes sequential 1.3 MB but **fails every parallel workload** (truncation/timeout; `wc -c != Content-Length`).
- Raw JSON: `proxy-comparison/results/20260609T203510Z/`
