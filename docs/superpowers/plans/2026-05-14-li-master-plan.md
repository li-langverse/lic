...
- [ ] Phase 7d — Execution decorators — **partial (#150, #167, #203, #387):** `@vectorized` on `for` → `ArraySimdScope`; **7d-b** lanes=4; **def `@parallel(disjoint=)`** inherits to nested `parallel for` (policy); **closed slice:** MIR proc tags (`mir_parallel_disjoint=`, `mir_vectorized_proc=`) + Lean `_par*` policy witnesses (`parallel_disjoint_lean_opaque_gap.sh`); **open:** iteration-independence Lean specs (**P-par**), Lean **P-dec**
