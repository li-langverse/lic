...
**Agent verification (2026-06-04):** run `code_implementer-1780604568263` — rebased PR [#758](https://github.com/li-langverse/lic/pull/758); `li_has_runnable_lic` + `LIC_ROOT` in `scripts/lib/lic-bin-select.sh`; WSL delegation when no runnable native lic (stale `build-wsl` GLIBC 2.38 vs host 2.36).
...
**Agent verification (2026-06-03):** run `code_implementer-1780502188023` — `PH_ML_PROGRAM_COMPLETE_INNER=1 bash scripts/ph-ml-program-complete-gates.sh` exit 0 (~122s) on Debian glibc 2.36 (native `build/compiler/lic/lic` preferred over stale `build-wsl`).
