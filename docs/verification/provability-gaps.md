...
| **G-vc** | Partial | **Phase 2e v1 gate green** (`scripts/check-phase-2e-gates.sh`): typed AutoVC, MIR `witnessed_ensures`/`mir_return_linked`, open-goals checker, discharge corpus. **Closed slice:** call-site `requires`, const-local discharge, `sqrt_open_bound`. Still open: opaque `vec3_dot`-style returns; loop vs closed-form `ensures` beyond closed corpus |
...
| **G-vc** | VC generation | Contracts → proof obligations | **Partial** — **Phase 2e v1 exit gate:** `scripts/check-phase-2e-gates.sh` (typed AutoVC, `vc_witness.cpp`, open-goals checker, discharge corpus); call-site `requires`, E0303/E0304/E0305; open: float `abs`, opaque returns | **2e** | `check-phase-2e-gates.sh`, `vc_emit_contracts.sh`, `mir_vc_witness.sh`, `check-autovc-open-goals.sh`, `contracts_discharge_corpus.sh`, `compiler/verify/vc_witness.cpp`, `prove_reject/weak_ensures_true.li` |
