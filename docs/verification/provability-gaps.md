...
| **Decorators (`@parallel`, …)** | Compile-time elaboration + proofs | **Partial** — parse + policy (7d-a/e); **closed slice:** MIR proc tags for `@cpu`/`@parallel`/`@vectorized`/`@gpu` + `lic verify` telemetry; Lean **P-dec** open |
...
| **G-dec** | Partial | **Closed slice:** MIR proc tags (`@cpu`, `@parallel(disjoint=)`, `@vectorized`, `@gpu`) + `lic verify` telemetry + `check-mir-*-decorator.sh` corpus; 4× `decorator_exploits` `compile_fail`; `@vectorized` on `for` (`vectorized_for_scope_ok.li`); Lean **P-dec** open |
...
| **G-dec** | Execution decorators | Static elaboration; reserved names; no runtime | **Partial** — **closed slice:** 4× `decorator_exploits` `compile_fail`; `@cpu`/`@parallel(disjoint=)`/`@vectorized`/`@gpu` MIR proc tags + `lic verify` telemetry; `@vectorized` on `for` (`vectorized_for_scope_ok.li`); `check-mir-{cpu,parallel,gpu,vectorized}-decorator.sh` in 2f corpus | **7d** | `contracts_discharge_corpus.sh`, `decorator_exploits/`, `check-mir-*-decorator.sh` |
...
- Decorator elaboration (**G-dec** — MIR proc tags wired; Lean **P-dec** open)  
...
| Decorators | Never interpreted at run time | **Partial** — MIR proc tags on `def` (`@cpu`, `@parallel`, `@vectorized`, `@gpu`); loop `@vectorized` → `ArraySimdScope`; Lean **P-dec** open |
