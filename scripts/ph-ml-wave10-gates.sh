...
run_in_wsl() {
  ...
  wsl.exe bash -lc "cd '$wsl_root' && export PH_ML_WAVE10_ROOT='$wsl_root' PH_ML_WAVE10_INNER=1 source scripts/ph-ml-wave10-gates.sh"
}
...
if [[ "${PH_ML_WAVE10_INNER:-0}" != "1" ]] && ! lic_is_runnable "$ROOT/build/compiler/lic/lic" \
...
lic_is_runnable "$LIC" || { echo "ph-ml-wave10-gates: build lic (./scripts/build.sh or WSL build-wsl)"; exit 1; }
