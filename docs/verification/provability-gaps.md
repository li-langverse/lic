...
| **Decorators (`@parallel`, …)** | Compile-time elaboration + proofs | **Partial** — parse + policy (7d-a/e); **closed slice:** MIR proc tags (`@cpu`, `@gpu`, `@parallel`, `@vectorized`, `@offload`); `@tpu`/`@serial`/`@async`/`decorator def` expansion open |
...
| **G-dec** | Partial | **Closed slice:** MIR proc tags (`mir_cpu_def`, `mir_gpu_def`, `mir_vectorized_proc`, `mir_parallel_disjoint`) + `decorator_exploits` CI; Lean **P-dec** + `decorator def` expansion open |
...
| **G-dec** | Execution decorators | Static elaboration; reserved names; no runtime | **Partial** — **closed slice:** 4× `decorator_exploits` `compile_fail`; `@vectorized` on `for` (`vectorized_for_scope_ok.li`); MIR proc tags (`@cpu`, `@gpu`, `@parallel`, `@vectorized`, `@offload`) + `check-mir-*-decorator.sh` corpus | **7d** | `contracts_discharge_corpus.sh`, `decorator_exploits/`, `scripts/check-mir-cpu-decorator.sh` |
...
flowchart LR
  ...
  subgraph missing [not wired]
    ...
    dec_exp[decorator def expansion]
  end
  subgraph partial [7d partial]
    dec_tags[MIR proc tags]
  end
  vc -.->|Phase 2f| lean
  par --> dec_tags
  dec_tags --> mir
  par -.->|7d-e open| dec_exp
```
...
| Decorators | Never interpreted at run time | **Partial** — MIR proc tags on `def`/`for`; `decorator def` expansion open |
...
| `li-tests/decorator_exploits/` | Policy **reject** reserved names, typosquat, missing `disjoint=` (7d-e); macro expansion open |
