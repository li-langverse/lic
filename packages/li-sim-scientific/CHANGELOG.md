...
- **WP-SCI-03 Phase 2 tier-2 CFD/FEA/QM** — `sim_scientific_oracle_checksum_cfd` (steady lid-channel sum 4.0), `fea_oracle_bar_chain_checksum`, `sim_scientific_oracle_checksum_qm_dft` (`chem_dft_energy_kernel_hartree`); `run_cfd_tier2_registry` (205–210), `run_fea_tier2_registry` (211–216), `run_qm_tier2_registry` (401–432); `vertical_cfd_lid_driven_cavity` / `vertical_fea_linear_elasticity` in `li-sim`; heat dispatch narrowed to 201–204; stub retained for drug/bio/ml/am/viz/robo/auto only.
...
- `run_md_lj_smoke` / `run_heat_smoke` use tier-2 oracle checksums (not scalar stub / constant 1.0); `li_sim_scientific_version` → 7 (WP-SCI-03 Phase 2 CFD/FEA/QM).
