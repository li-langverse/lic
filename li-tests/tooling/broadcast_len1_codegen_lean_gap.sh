#!/usr/bin/env bash
# G-math / PH-2i: length-1 broadcast — Lean Prop + eval in Discharge.lean; MIR/codegen unchanged.
# Contrast: mat2_at2_mir_codegen_lean_gap.sh (MIR↔eval lemma still open for `@`).
set -euo pipefail
...
if ! grep -q 'broadcast_rhs_len1_add_float4_spec' "$DISCHARGE"; then
  echo "FAIL: Discharge.lean should define broadcast_rhs_len1_add_float4_spec" >&2
  exit 1
fi
if ! grep -q 'witness_broadcast_rhs_len1_add_float4_spec' "$WITNESS"; then
  echo "FAIL: vc_witness.cpp should wire broadcast_len1 witness" >&2
  exit 1
fi
if ! grep -q 'broadcast_rhs_len1_add_float4_spec_proved' "$VC_EMIT"; then
  echo "FAIL: vc_emit_lean.cpp should emit broadcast_len1 discharge theorem" >&2
  exit 1
fi
...
if ! grep -A2 'broadcast_len1_add_float4' "$MANIFEST" | grep -q 'compile_ok'; then
  echo "FAIL: broadcast_len1_add_float4 smoke should remain compile_ok" >&2
  exit 1
fi
...
rm -f "$AUTOVC"
"$LIC" build "$CLOSED" -o /dev/null 2>/dev/null
if ! grep -qi 'broadcast_rhs_len1_add_float4' "$AUTOVC"; then
  echo "FAIL: AutoVC should reference Li.Discharge.broadcast_rhs_len1_add_float4" >&2
  exit 1
fi
if ! grep -q 'broadcast_rhs_len1_add_float4_spec_proved' "$AUTOVC"; then
  echo "FAIL: AutoVC should cite broadcast_rhs_len1_add_float4_spec_proved" >&2
  exit 1
fi
...
echo "PASS broadcast_len1_codegen_lean_gap: Lean spec + witness closed; MIR/codegen OK; compile_ok smoke retained"
