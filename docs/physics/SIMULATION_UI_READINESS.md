...
2. **Import graph** — composable `import_physics_runtime.li` smoke-tests `import physics.runtime` + `physics_step`; extend with direct `RigidBody` fixtures when imported object types compile in `li-tests/composable/`.
