...
| **G-math** | Partial | **Closed slice (tier-1):** `matmul_naive` ≤1.2× C++ (`check-tier1-li-vs-cpp.sh`, loop matmul). **Open (tier-1):** `horner_pure_li` — scalar FMA `HornerConstLoopF64` path landed ([#11](https://github.com/li-langverse/lic/issues/11)); dashboard/ingest ≤1.2× gate pending measured green (do not claim closed slice until ingest). **Closed slice:** full 2×2 float `@` Lean Prop (`linalg_mat2_at2_float_closed`, `mat2_at2_float_spec`). **Closed slice:** `linalg_dot4_float_closed` (prelude `dot`), `linalg_mat2_callproc_float_closed`, prelude `norm`/`axpy`/**, IKJ `ArrayMatMul2DF64` enforced only with `LI_TIER1_PERF_STRICT=1` (`check-tier1-li-vs-cpp.sh` reports gaps by default). **Closed slice:** prelude `norm`, `axpy`, same-length `**`, scalar×array, `math_linalg/reductions/`, loop-dot witness, P-linalg corpus |
