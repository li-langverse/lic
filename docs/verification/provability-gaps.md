...
| **G-par** | Partial | AST `policy_module` rejects missing disjoint, false `disjoint_row` on `grid[0][0]`, mut capture, borrow-in-par; **open:** `disjoint_row` + `grid[i][0]` still accepted (`disjoint_row_writes_row_i.li`); Lean proofs open |
...
| **G-par** | `parallel for` safety | Proved iteration independence | **Partial** — AST `policy_module` rejects `grid[0][0]` under `disjoint_row`; **open** `grid[i][0]` alias (`disjoint_row_writes_row_i.li`); string heuristics in `policy.cpp` | **7b**, **7d-c** | `race_shared_memory/`, `policy_disjoint_row_soundness.sh`, `decorator_exploits` |
