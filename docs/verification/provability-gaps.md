...
| **Decorators (`@parallel`, …)** | Compile-time elaboration + proofs | **Partial** — **closed slice:** `@cpu`/`@parallel` → Host `OmpParallelFor` → `li_parallel_for_i64`; `@gpu` placement metadata; Device LKIR open |
...
| **G-dec** | Partial | **Closed slice (7d-b–e):** `check-mir-decorator-lowering.sh` — `@vectorized`/`@cpu`/`@gpu`/`@parallel` MIR tags, Host `li_parallel_for_i64`, `decorator_exploits` 4× `compile_fail`; Lean **P-dec** + Device LKIR (**G-gpu**) open |
...
| **G-dec** | Execution decorators | Static elaboration; reserved names; no runtime | **Partial** — **closed slice (7d-b–e):** unified gate `check-mir-decorator-lowering.sh`; 4× `decorator_exploits` `compile_fail`; `@vectorized` on `for` (`vectorized_for_scope_ok.li`); `@cpu`/`@parallel` Host memory-space MIR + `li_parallel_for_i64` codegen; `@gpu` placement telemetry; `std.execution.parallel` policy surface. **G-par** cross-link: `disjoint=` Host lowering shares `check-mir-parallel-portable-lowering.sh` | **7d**, **7e** | `check-mir-decorator-lowering.sh`, `decorator_exploits/`, `contracts_discharge_corpus.sh` |
...
| Decorators | Never interpreted at run time | **N/A** — not executed; **7d-b–e closed slice** elaborates to MIR tags + Host parallel lowering (**G-dec** partial) |
