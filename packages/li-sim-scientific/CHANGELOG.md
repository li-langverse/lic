...
- **WP-SCI-03 tier-2 CFD/FEA dispatch** — `sim_scientific_oracle_checksum_cfd`, `sim_scientific_oracle_checksum_fea`, `run_cfd_tier2_registry`, `run_fea_tier2_registry`; split PDE registry rows 205–210 (CFD) and 211–216 (FEA) from heat oracle; `vertical_cfd_lid_driven_cavity`, `vertical_fea_linear_elasticity`; extend `run_algo_registry_tier2.li`.
- **WP-SCI-03 CFD/FEA real kernels** — replace scaffold checksums (3.0/4.0) with `cfd_lid_cavity_step` diffusion oracle and `fea_truss_series_displacement` elasticity witness.

### Changed

- `run_md_lj_smoke` / `run_heat_smoke` use tier-2 oracle checksums (not scalar stub / constant 1.0); `li_sim_scientific_version` → 7 (WP-SCI-03 CFD/FEA).
