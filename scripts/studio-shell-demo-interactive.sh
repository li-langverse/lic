...
if [[ ! -x "$BIN" ]]; then
  ...
  "$LIC" build --allow-open-vc --no-lean-verify \
    "$ROOT/packages/li-studio/src/main.li" -o "$BIN"
fi
...
echo "studio-shell-demo-interactive: bin=$BIN profile=$STUDIO_DEMO_PROFILE lig_host_present=$LIG_HOST_PRESENT tick_ms=$TICK_MS" >&2
