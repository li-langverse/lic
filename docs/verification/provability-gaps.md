...
| **G-gpu** | Partial | **Closed slice:** `@gpu` MIR telemetry; **Wave 13:** `ml_gpu_device_buffer_pipeline` + `lig_gpu_device_buffer_ready` host contract. **#110:** cross-space read without `@sync_host`/`@sync_device` is open obligation (Kokkos DualView deprecation alignment). Still open: address-space proofs, full LKIR lowering |
...
| **G-gpu** | `@gpu` / device buffers | Separate address space proofs | **Partial** — MIR telemetry + **Wave 13** device-buffer host contract; **#110:** cross-space sync obligation documented (`std/execution/spaces.li`, `docs/hpc/kokkos-memory-execution-spaces-rubric.md`); address-space proofs open | **3+**, **7d**, **7e** | `li-tests/decorators/gpu_*`, `li-tests/execution/spaces_smoke.li`, `packages/li-ml/li-tests/smoke/ml_gpu_device_buffer.li`, `scripts/check-mir-gpu-decorator.sh` |
