...
- [ ] Phase 2i — Math / linalg surface — **partial:** **2i-a/c** (#148); **2i-b** `norm`, `sum`/`dot`, `reductions/`, same-length `**`, prelude `axpy`, scalar×array; **length-1 broadcast done** (`array[1]`→`array[N]` element-wise, `broadcast_len1_*.li`); **full NumPy rank broadcast deferred** ([#526](https://github.com/li-langverse/lic/issues/526), reject gate + 2-rank `compile_fail` seeds); float `@` Props closed (`mat2_at2_eval`)
