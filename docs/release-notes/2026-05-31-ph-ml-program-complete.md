...
**Agent verification (2026-06-04):** run `code_implementer-84435465` — rebased PR [#783](https://github.com/li-langverse/lic/pull/783) onto `main`; `scripts/lib/lic-bin-select.sh` gains `li_has_runnable_lic` for WSL delegation when native `build/compiler/lic` is missing or not loadable (stale `build-wsl` glibc).
