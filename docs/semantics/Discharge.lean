...
/-!
## Length-1 broadcast (**PH-2i** / BUG-C-03)

Element-wise binops on `array[N, T]` × `array[1, T]` reuse index 0 of the shorter operand
(`array_broadcast_*_len1` in MIR). Closed fixed-size Props for N = 4.
-/

/-- `array[4, float] + array[1, float]` broadcast (`broadcast_len1_add_float4_closed.li`). -/
...
/-- `array[4, int] * array[1, int]` broadcast (`broadcast_len1_mul_int4_closed.li`). -/
def broadcast_len1_mul_int4_spec (a : LiArray Int 4) (b : LiArray Int 1) (result : LiArray Int 4) :
    Prop :=
  (result[0]! = a[0]! * b[0]!) ∧
  (result[1]! = a[1]! * b[0]!) ∧
  (result[2]! = a[2]! * b[0]!) ∧
...
/-- `array[4, int] ** array[1, int]` broadcast (`broadcast_len1_pow_int4_closed.li`). -/
def broadcast_len1_pow_int4_spec (a : LiArray Int 4) (b : LiArray Int 1) (result : LiArray Int 4) :
    Prop :=
  (result[0]! = a[0]! ^ Int.toNat b[0]!) ∧
  (result[1]! = a[1]! ^ Int.toNat b[0]!) ∧
  (result[2]! = a[2]! ^ Int.toNat b[0]!) ∧
...
theorem broadcast_len1_pow_int4_spec_proved (a : LiArray Int 4) (b : LiArray Int 1) :
  ...
  unfold broadcast_len1_pow_int4_spec broadcast_len1_pow_int4_eval
  refine And.intro rfl (And.intro rfl (And.intro rfl rfl))
