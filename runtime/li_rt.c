...
static int32_t li_rt_lig_backend_match_name(const char* name) {
  ...
  if (strcmp(name, "cuda") == 0) return LI_RT_LIG_BACKEND_CUDA;
  if (strcmp(name, "rocm") == 0 || strcmp(name, "hip") == 0) return LI_RT_LIG_BACKEND_ROCM;
  if (strcmp(name, "metal") == 0) return LI_RT_LIG_BACKEND_METAL;
  if (strcmp(name, "webgpu") == 0 || strcmp(name, "wgpu") == 0) return LI_RT_LIG_BACKEND_WEBGPU;
  return 0;
}

int32_t li_rt_lig_device_kind(void) { return g_lig_selected_backend; }
int32_t li_rt_lig_backend_available(int32_t backend_id) { return li_rt_lig_backend_probe_available(backend_id); }
...
/* PH-HW HW-1 — lig.present trusted edge. */
...
int32_t li_rt_lig_parse_toml_backend_line(const char* line) {
  ...
  size_t n = 0;
  if (*p == '"') { p++; start = p; while (*p && *p != '"') p++; if (*p != '"') return 0; n = (size_t)(p - start); }
  else { start = p; while ((*p >= 'a' && *p <= 'z') || (*p >= '0' && *p <= '9') || *p == '_') p++; n = (size_t)(p - start); }
  if (n == 0 || n >= sizeof(buf)) return 0;
  memcpy(buf, start, n); buf[n] = ' ';
  const int32_t id = li_rt_lig_backend_match_name(buf);
  if (id != 0) g_lig_selected_backend = id;
  return id;
}

int32_t li_rt_studio_shell_input_pointer_down(void) { return g_studio_shell_pointer_down; }
float li_rt_studio_shell_input_pointer_x(void) { return g_studio_shell_pointer_x; }
float li_rt_studio_shell_input_pointer_y(void) { return g_studio_shell_pointer_y; }
int32_t li_rt_studio_shell_input_key_escape(void) { return g_studio_shell_key_escape; }
int32_t li_rt_studio_shell_input_key_cmd_k(void) { return g_studio_shell_key_cmd_k; }
int32_t li_rt_studio_shell_input_key_digit(void) { return g_studio_shell_key_digit; }
