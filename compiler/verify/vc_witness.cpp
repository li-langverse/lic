...
bool expr_is_broadcast_rhs_len1_entry_eq(const Expr* e, const std::string& c, const std::string& a,
                                         const std::string& b, std::int64_t idx) {
  if (e == nullptr || e->kind != Expr::Kind::BinOp || e->bin_op != BinOp::Eq || !e->lhs ||
      !e->rhs) {
    return false;
  }
  ...
  const auto a_idx_b0 = [&](const Expr* x, const Expr* y) -> bool {
    return x && x->kind == Expr::Kind::Index && expr_is_ident(x->base.get(), a) &&
           expr_is_int_lit(x->index.get(), idx) && y && y->kind == Expr::Kind::Index &&
           expr_is_ident(y->base.get(), b) && expr_is_int_lit(y->index.get(), 0);
  };
  return a_idx_b0(rhs->lhs.get(), rhs->rhs.get()) || a_idx_b0(rhs->rhs.get(), rhs->lhs.get());
}

bool expr_is_broadcast_rhs_len1_add_float4_spec(const Expr& e, const std::string& a,
  ...
  for (std::int64_t idx = 0; idx < 4; ++idx) {
    if (!expr_is_broadcast_rhs_len1_entry_eq(terms[static_cast<std::size_t>(idx)], c, a, b, idx)) {
      return false;
    }
  }
  return true;
...
bool witness_broadcast_rhs_len1_add_float4_spec_impl(const ProcDecl& proc,
  ...
  return expr_is_broadcast_rhs_len1_add_float4_spec(ensures_expr, ret->lhs->ident, ret->rhs->ident,
                                                    "result");
}
...
bool ensures_witnessed_for_return(const ProcDecl& proc, const Contract& c, const Expr& ret,
  ...
  if (witness_broadcast_rhs_len1_add_float4_spec_impl(proc, *c.expr)) {
    return true;
  }
...
bool witness_broadcast_rhs_len1_add_float4_spec(const ProcDecl& proc, const Expr& ensures_expr) {
  return witness_broadcast_rhs_len1_add_float4_spec_impl(proc, ensures_expr);
}
