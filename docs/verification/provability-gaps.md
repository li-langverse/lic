...
| **G-vc** | Partial | **v1 exit (2e-a…c):** typed `AutoVC.lean`; `lic verify` prints `witnessed_ensures=` + `mir_return_linked=` (`vc_witness.cpp`); `check-phase-2e-gates.sh` green. **Closed slice:** `sqrt_open_bound` float `abs` bound (trusted libm axiom). **Still open (2e-d):** opaque `vec3_dot`-style returns; loop implementations vs closed-form `ensures` |
...
| **G-vc** | VC generation | Contracts → proof obligations | **Partial** — **v1 exit:** `check-phase-2e-gates.sh` (2e-a `vc_emit_contracts.sh`, 2e-b `mir_vc_witness.sh` + `vc_witness.cpp`, 2e-c `contracts_discharge_corpus.sh` + `check-autovc-open-goals.sh`); **closed slice:** call-site `requires`, const-local discharge, E0303/E0304/E0305, `sqrt_open_bound`; **open (2e-d):** opaque returns, loop vs closed-form | **2e** | [phase-02e plan](../superpowers/plans/2026-05-14-phase-02e-contracts.md), `check-phase-2e-gates.sh`, `vc_emit_contracts.sh`, `mir_vc_witness.sh`, `discharge_caller_requires_lean.sh`, `discharge_caller_requires_local_lean.sh`, `contracts_discharge_corpus.sh`, `prove_reject/weak_ensures_true.li` |
