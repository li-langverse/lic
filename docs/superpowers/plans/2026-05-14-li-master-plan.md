...
- [ ] Phase 7d — Execution decorators — **partial (lic#2):** **7d-b–e closed slice** — `check-mir-decorator-lowering.sh` (`@vectorized`/`@cpu`/`@gpu`/`@parallel` MIR tags, Host `li_parallel_for_i64`, `decorator_exploits`); **open:** Tier 2 MD `@` example, Lean **P-dec**/**G-par** proofs, Device LKIR (**G-gpu**)
