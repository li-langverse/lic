...
| **Decorators (`@parallel`, …)** | Compile-time elaboration + proofs | **Partial** — parse + policy (7d-a/e); **closed slice (7d-b/c):** proc `@parallel(disjoint=)` + `@vectorized` MIR tags; `@vectorized for` scoped (#150); **open:** 7d-e macro expansion — [plan](../superpowers/plans/2026-06-07-ph-7d-g-dec-mir-elaboration.md) ([#22](https://github.com/li-langverse/lic/issues/22)) |
...
| **G-dec** | Partial | **Closed slice (7d-b/c):** proc `@parallel(disjoint=)` → `mir_parallel_disjoint`; proc `@vectorized` → `ArraySimdScope` (`mir_vectorized_def_scope`); `@vectorized for` scoped; `decorator_exploits/` CI; Lean **P-dec** open — [plan](../superpowers/plans/2026-06-07-ph-7d-g-dec-mir-elaboration.md) ([#22](https://github.com/li-langverse/lic/issues/22)) |
...
| **G-dec** | Execution decorators | Static elaboration; reserved names; no runtime | **Partial** — **closed slice (7d-b/c):** proc `@parallel(disjoint=)` + `@vectorized` MIR elaboration (`mir_parallel_disjoint`, `mir_vectorized_def_scope`); `@vectorized` on `for` (`vectorized_for_scope_ok.li`); 4× `decorator_exploits` `compile_fail` | **7d** | `check-mir-parallel-decorator.sh`, `check-mir-vectorized-decorator.sh`, `decorator_exploits/` |
