...
- **WP-SCI-03 CFD/FEA tier-2 registry slice** — `sim_scientific_oracle_checksum_cfd`, `sim_scientific_oracle_checksum_fea`, `run_cfd_tier2_registry` (algo 205–210), `run_fea_tier2_registry` (211–216); PDE heat dispatch narrowed to 201–204; `vertical_cfd_lid_driven_cavity` / `vertical_fea_linear_elasticity` in `sim`; smokes extended in `run_algo_registry_tier2.li` and `scientific_oracle_bench.li`.
...
- `li_sim_scientific_version` → 7 (WP-SCI-03 CFD/FEA tier-2 slice); `run_md_lj_smoke` / `run_heat_smoke` use tier-2 oracle checksums (not scalar stub / constant 1.0).
