...
- **WP-SCI-03 Phase 2 QM/FEA/CFD tier-2** — `sim_scientific_oracle_checksum_qm` (`chem_dft_energy_kernel_hartree`), `sim_scientific_oracle_checksum_fea`, `sim_scientific_oracle_checksum_cfd`; `run_qm_tier2_registry` (401–432), `run_fea_tier2_registry` (701–703), `run_cfd_tier2_registry` (704–707); `vertical_fea_linear_elasticity` / `vertical_cfd_lid_driven_cavity` + `algo_fea_linear_elasticity` (701) / `algo_cfd_lid_driven_cavity` (704) in `li-sim`.
...
- `run_md_lj_smoke` / `run_heat_smoke` use tier-2 oracle checksums (not scalar stub / constant 1.0); `li_sim_scientific_version` → 7 (WP-SCI-03 Phase 2 QM/FEA/CFD).
