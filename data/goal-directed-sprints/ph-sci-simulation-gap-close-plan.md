...
- **Remaining lib gaps (Phase 2):** `run_algo_registry_stub` for QM 401–432; `li-sim-viz` compose-only; scene `native_pixels` stub; particles MD force accumulation still weak. CFD 205–210 + FEA 211–216 now tier-2 oracles (WP-SCI-03 partial).
...
| **li-sim-scientific** | partial → tier-2 oracle | CPU + `@gpu` MD oracle smoke | Lib builds with open VC; smokes verify | **Real:** 4-particle LJ chain + 1D heat + CFD lid-cavity + FEA bar oracles, `run_multi_physics_at_step`. **Placeholder:** `run_algo_registry_stub` (checksum 1.001) for QM 401–432 | WP-SCI-03 partial (QM stub); no LAMMPS/GROMACS external oracle |
