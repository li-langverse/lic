...
- **WP-SCI-03 unified tier-2 registry (CFD/FEA/QM)** — `sim_scientific_oracle_checksum_cfd`, `sim_scientific_oracle_checksum_fea`, `sim_scientific_oracle_checksum_qm`, `run_cfd_tier2_registry`, `run_fea_tier2_registry`, `run_qm_tier2_registry`; split PDE registry rows 201–204 heat / 205–210 CFD / 211–216 FEA; QM rows 401–432 harmonic-well oracle; extended smoke `run_algo_registry_tier2.li`.
...
- `run_md_lj_smoke` / `run_heat_smoke` use tier-2 oracle checksums (not scalar stub / constant 1.0); `li_sim_scientific_version` → 8 (WP-SCI-03 CFD/FEA/QM unified).
