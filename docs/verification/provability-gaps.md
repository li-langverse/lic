...
| **G-vc** | Partial | **Phase 2e partial exit gate green:** `check-phase-2e-exit-gates.sh` (`vc_emit_contracts.sh`, `mir_vc_witness.sh`, `contracts_discharge_corpus.sh`, `vc_witness.cpp`, `check-autovc-open-goals.sh`). **Closed slice:** call-site `requires`, const-local discharge, `sqrt_open_bound`. **Still open:** opaque `vec3_dot`-style returns; loop implementations vs closed-form `ensures` beyond closed corpus |
...
| **G-vc** | VC generation | Contracts → proof obligations | **Partial** — **2e partial exit gate:** `./scripts/check-phase-2e-exit-gates.sh` (typed AutoVC, MIR witnesses, zero open goals on closed corpus + open probe). **Closed slice:** call-site `requires`, const-local discharge, E0303/E0304/E0305, `sqrt_open_bound`. **Open:** opaque returns, **P-ensures-witness** | **2e** | `check-phase-2e-exit-gates.sh`, `vc_emit_contracts.sh`, `mir_vc_witness.sh`, `compiler/verify/vc_witness.cpp`, `contracts_discharge_corpus.sh`, `check-autovc-open-goals.sh`, `discharge_caller_requires_lean.sh`, `prove_reject/weak_ensures_true.li` |
...
| `scripts/check-phase-2e-exit-gates.sh` | **Phase 2e partial gate** — bundles VC emit, MIR witness, discharge corpus |
| `li-tests/tooling/vc_emit_contracts.sh` | `sqrt_contract` AutoVC uses `≥` / `Float.abs`, not `True` stubs |
| `li-tests/tooling/mir_vc_witness.sh` | `lic verify` reports `witnessed_ensures=` + `mir_return_linked=` |
| `compiler/verify/vc_witness.cpp` | C++ return-shape witnesses for MIR-linked `ensures` |
