...
| **G-par** | Partial | AST `policy_module` rejects missing disjoint, false `disjoint_row` (`grid[0][0]`), mut capture, borrow-in-par; **`disjoint_elem` + `buf[0]` still accepted** (`policy_disjoint_elem_soundness.sh`); Lean proofs open |
| **G-dec** | Partial | Decorator elaboration to MIR; **7d-b** `@vectorized` on `for` → `ArraySimdScope`; `@parallel` on plain `for` stays serial (`parallel_decorator_for_elaboration_gap.sh`); `@parallel` without `disjoint=` on `for` not policy-checked |
...
| **G-par** | `parallel for` safety | Proved iteration independence | **Partial** — AST `check_module_policies`; `disjoint_row` + `grid[0][0]` rejected; **`disjoint_elem` + `buf[0]` hole** | **7b**, **7d-c** | `race_shared_memory/`, `policy_disjoint_elem_soundness.sh`, `decorator_exploits` |
