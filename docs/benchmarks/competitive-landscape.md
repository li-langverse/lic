...
## Domain MD oracles (external_binary)

Planned **`lammps`** / **`gromacs`** validity columns for `md_lennard_jones` — not language runtimes. Registry: `benchmarks/competitive/md_oracle.toml`. Plan: [competitive-engines-plan.md](competitive-engines-plan.md). CI stub: `scripts/md-external-oracle-stub.py` / `li-tests/tooling/md_external_oracle_stub.sh`.

| Engine | CSV `lang` | Status |
|--------|------------|--------|
| LAMMPS | `lammps` | stub (B1 deck) |
| GROMACS | `gromacs` | stub (B2 mdrun) |

Do not compare Li/cpp wall time to LAMMPS until workloads match `md_core.c` IC and LJ parameters.
