...
| **Decorators (`@parallel`, …)** | Compile-time elaboration + proofs | **Partial** — parse + policy; `@vectorized` on `def` → MIR proc tag (7d-b) |
...
| **G-dec** | Partial | `MirDecorator.vectorized` proc tag; `lic verify mir_vectorized_proc=`; `@parallel` MIR on `feat/fresh-gpar-7d` |
...
| **G-dec** | Execution decorators | Static elaboration; reserved names; no runtime | **Partial** — **closed slice:** 4× `decorator_exploits` `compile_fail`; `MirDecorator.vectorized` + `lanes` on `def`; `lic verify mir_vectorized_proc=`; `@parallel` MIR on `feat/fresh-gpar-7d` | **7d** | `decorator_exploits/`, `vectorized_dot_proc_ok.li`, `check-mir-vectorized-decorator.sh` |
