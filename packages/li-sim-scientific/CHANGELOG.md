...
- **WP-SCI-03 CFD/FEA tier-2 registry** — `sim_scientific_oracle_checksum_cfd`, `sim_scientific_oracle_checksum_fea`, `run_cfd_tier2_registry` (205–210), `run_fea_tier2_registry` (211–216); heat dispatch narrowed to 201–204; smokes `run_algo_registry_tier2_cfd_fea.li`; `vertical_cfd_lid_driven_cavity` / `vertical_fea_linear_elasticity` + algo id helpers in `li-sim`.
...
- `run_heat_tier2_registry` covers PDE heat rows **201–204** only (CFD/FEA split to tier-2 oracles); `li_sim_scientific_version` → 7.
