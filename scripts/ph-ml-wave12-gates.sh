...
run_in_wsl() {
  ...
  wsl.exe bash -lc "cd '$wsl_root' && PH_ML_WAVE12_ROOT='$wsl_root' PH_ML_WAVE12_INNER=1 LIG_EMIT_CUDA=1 BENCHMARKS_ROOT='${wsl_bench}' bash scripts/ph-ml-wave12-gates.sh"
}

lic_bin_for_smokes() {
  lic_rel_for_smokes "$1" "$ROOT"
}
...
if [[ "${PH_ML_WAVE12_INNER:-0}" != "1" ]] && ! lic_is_runnable "$ROOT/build/compiler/lic/lic" \
...
lic_is_runnable "$LIC" || { echo "ph-ml-wave12-gates: build lic (./scripts/build.sh or WSL build-wsl)"; exit 1; }
LIC="$(lic_rel_for_smokes "$LIC" "$ROOT")"
