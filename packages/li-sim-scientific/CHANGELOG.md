...
- **WP-SCI-03 Phase 2 tier-2 oracles** — `sim_scientific_oracle_checksum_qm` / `_qm_overlap` (SPH overlap proxy), `_cfd` (SPH poly6 kernel row), `_fea` (scalar stiffness proxy); `run_qm_tier2_registry` (401–432), `run_cfd_tier2_registry` (205–210), `run_fea_tier2_registry` (211–216); heat dispatch narrowed to 201–204; `vertical_fea_linear_elasticity` / `vertical_cfd_lid_driven_cavity` in `li-sim`; smoke `run_algo_registry_tier2.li` extended for CFD/FEA/QM rows.
...
- `li_sim_scientific_version` → 7 (WP-SCI-03 Phase 2 QM/CFD/FEA tier-2 dispatch); align package smokes with version 7.
- `run_md_lj_smoke` / `run_heat_smoke` use tier-2 oracle checksums (not scalar stub / constant 1.0); prior bump to 6 (WP-ECHEM-15).
