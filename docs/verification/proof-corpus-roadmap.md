...
| `sqrt_open_bound.li` | `abs(result² - x) < ε` with `li_rt_sqrt` body | **Closed** — `prove_lean_ok`; `Li.Discharge.sqrt_open_bound_spec` (trusted libm) |
...
| `contracts_discharge_corpus.sh` | **ok** | Trivial/const/index/caller-requires/**linalg closed**; `sqrt_open_bound` closed; loop dot closed |
