...
**Publish target:** `research-findings/whitepapers/2026-05/swarm_coverage/performance/`

---

## Abstract

This pass audits swarm health through a **performance lens**: benchmark posture, performance-related gap registry rows, and orchestration blockers that prevent unattended gap closure. The Li tier-1 matrix shows **no red rows** (146 green, 2 yellow, 5 near-threshold), but the swarm remains **degraded (conditional)** because control-plane telemetry is incomplete, PyYAML is absent from the worker image, and briefing-recommended leaf agents diverge from the scorecard — notably **no `bench_improver` dispatch** despite actionable near-threshold numerics.
...
- httpd wrk: `gap-phase2-perf-wrk-soak`, `gap-phase2-streaming-wrk` — pending; exit 124 history (snapshot stale)
...
|---------|---------------------|-------------|
| PyYAML missing | Gap registry stale → wrong bench handoffs | Bake in org-research image |
...
| Briefing drift | `gap_explorer` / `bench_improver` not dispatched | Align heap with scorecard |

---

## Recommendations

1. **P0:** Bake PyYAML in worker image; re-run ingest/apply on every cycle.
...
- Observer report: `/app/data/runs/swarm_observer-1781193991102.md`
- Orchestrator note: `/workspace/lic/docs/ecosystem/orchestrator-notes/2026-06-11-orch-performance-3c448d76.md`
