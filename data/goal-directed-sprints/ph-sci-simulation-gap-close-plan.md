...
- **Remaining lib gaps (Phase 2):** `run_algo_registry_stub` for CFD/FEA rows (501+ drug/AM/auto still stub); QM ids 401–432 tier-2 via `chem_dft_gpu_energy_checksum` (2026-06-08); `li-sim-viz` compose-only; scene `native_pixels` stub; particles MD force accumulation still weak.
