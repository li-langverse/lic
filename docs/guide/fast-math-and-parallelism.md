...
!!! note "Status"
    **Math surface** (`dot`, `a @ b`, 2d `C = A @ B`, `sum`) is **implemented** on fixed `array` tiles. **Decorators** (`@vectorized`, `@parallel` on `def`) parse but do not fully elaborate yet. Parallel disjointness uses **heuristic** checks today. See **[Provability gaps](../verification/provability-gaps.md)**.
