# Orchestrator note — performance gap orchestration (`089f5bce`)
...
| Gap prep | **Unblocked this run** — `apt install python3-yaml`; ingest + apply @ `00:05:46Z` |
...
| Unattended? | **Conditional** — gap scripts work after PyYAML; image bake + CP persist still needed |
...
```bash
apt-get install -y python3-yaml   # ephemeral — bake into image
cd /workspace/lic
python3 scripts/swarm-gap-ingest.py    # 92 registry rows; updated_at 00:05:46Z
...
- `/app/data/runs/swarm_observer-1781134901690.md`
