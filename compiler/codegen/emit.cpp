...
struct EmitCtx {
  ...
  void emit_matmul2d_init_abc(llvm::AllocaInst* c_mat, llvm::AllocaInst* a_mat,
                            llvm::AllocaInst* b_mat, unsigned m, unsigned n) {
    llvm::Type* f64 = llvm::Type::getDoubleTy(context);
    llvm::Type* i32t = i32_ty(context);
    ...
    llvm::Value* f001 = llvm::ConstantFP::get(f64, 0.01);
    llvm::Value* f002 = llvm::ConstantFP::get(f64, 0.02);
    llvm::Value* c17 = llvm::ConstantInt::get(i32t, 17);
    llvm::Value* c13 = llvm::ConstantInt::get(i32t, 13);
    llvm::Value* c3 = llvm::ConstantInt::get(i32t, 3);
    ...
    emit_idx_for(i_s, lim_m, [&](llvm::Value* i) {
      emit_idx_for(j_s, lim_n, [&](llvm::Value* j) {
        ...
        llvm::Value* rem_a = builder->CreateURem(sum, c17);
        llvm::Value* av =
            builder->CreateFMul(f001, builder->CreateUIToFP(rem_a, f64));
        builder->CreateStore(av, matmul_gep2d(a_mat, i, j));
        llvm::Value* prod = builder->CreateAdd(builder->CreateMul(i, c3), j);
        llvm::Value* rem_b = builder->CreateURem(prod, c13);
        llvm::Value* bv =
            builder->CreateFMul(f002, builder->CreateUIToFP(rem_b, f64));
  ...
  void emit_matmul2d_blocked_ijk(llvm::AllocaInst* c_mat, llvm::AllocaInst* a_mat,
    ...
    emit_matmul2d_init_abc(c_mat, a_mat, b_mat, n, n);
        ...
        emit_idx_for_step(jj_s, lim_n, step, [&](llvm::Value* jj) {
          ...
          emit_range_for(i_s, ii, i_max, [&](llvm::Value* i) {
            emit_range_for(k_s, kk, k_max, [&](llvm::Value* k) {
              ...
              if (vectorize_j) {
                llvm::Value* step8 = llvm::ConstantInt::get(i32t, 8);
                emit_range_for_step(j_s, jj, j_max, step8, [&](llvm::Value* j) {
                  store_c_vec4(i, k, j, aik);
                  llvm::Value* j2 = builder->CreateAdd(j, vec_step);
                  store_c_vec4(i, k, j2, aik);
                });
              } else {
