...
- **WP-SCI-03 CFD/FEA/QM tier-2 registry** — `sim_scientific_oracle_checksum_cfd` (SPH poly6 kernel row), `sim_scientific_oracle_checksum_fea` (bar truss scaffold), `sim_scientific_oracle_checksum_qm_dft` (`chem_dft_scf_iteration_scaffold`); `run_cfd_tier2_registry` / `run_fea_tier2_registry` / `run_qm_tier2_registry` for algo ids 205–210, 211–216, 401–432; `vertical_cfd_lid_driven_cavity` / `vertical_fea_linear_elasticity` in `li-sim`; extended `run_algo_registry_tier2.li`.
