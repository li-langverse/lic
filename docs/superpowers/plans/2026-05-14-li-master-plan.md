...
**Problem:** Full `li-tests` + `scripts/local-ci.sh` often take **5–15+ minutes** on a devbox because many gates still run **`lic build` sequentially** (workspace members, single-TU frontend). **8p-a shipped:** `run_all.sh` uses **`LI_TEST_JOBS`** / `-j N` with per-worker `LI_BUILD_DIR`. Remaining wins: workspace pool (**8p-b**), compiler `--jobs` (**8p-c**), wall-time SLO (**8p-d**).
...
| **`li-tests/run_all.sh`** | **Done** — `LI_TEST_JOBS` / `-j N`, isolated `build/li-test-<id>/` ([#186](https://github.com/li-langverse/lic/pull/186), [#200](https://github.com/li-langverse/lic/pull/200)) | Keep; `LI_TEST_JOBS=1` for bisect |
...
| Sub | Task | Exit gate |
|-----|------|-----------|
| **8p-a** | **Parallel test orchestration** — **shipped** — `run_all.sh` accepts `-j N` / `LI_TEST_JOBS`; each worker gets isolated `build/` subtree so AutoVC/Lean do not stomp | **196/196** manifest green with `-j8` on Linux; `LI_TEST_JOBS=1` reproduces current sequential logs |
...
- [ ] Phase 8p — Parallel compile + CI throughput — **8p-a shipped:** parallel `run_all` + isolated `LI_BUILD_DIR` ([#186](https://github.com/li-langverse/lic/pull/186), [#200](https://github.com/li-langverse/lic/pull/200), `li-tests/tooling/run_all_parallel_smoke.sh`); **8p-c partial:** `ResourceOptions` + `lic build --jobs` reserved pass; **8p-d partial:** CI test-job smokes (`scripts/ci.sh`); **open:** 8p-b workspace pool (`LI_WORKSPACE_JOBS`), wall-time SLO ([#385](https://github.com/li-langverse/lic/issues/385)) ([§ 8p](#phase-8p--parallel-compile--ci-throughput))
...
| **8p** | — | **8p-a shipped** (parallel `run_all`); **open:** workspace pool (8p-b), `lic --jobs` frontend (8p-c), wall-time SLO (8p-d, [#385](https://github.com/li-langverse/lic/issues/385)) |
