...
if [[ -z "${LIC:-}" ]]; then
  LIC="$(lic_ph_ml_bin "$ROOT")" \
    || { echo "ph-ml-wave5-gates: build lic (./scripts/build.sh --build-dir build-wsl in WSL)"; exit 1; }
fi
[[ -x "$LIC" ]] || { echo "ph-ml-wave5-gates: build lic (./scripts/build.sh --build-dir build-wsl in WSL): LIC not executable: $LIC"; exit 1; }
