...
1. [provability-gaps.md](../../verification/provability-gaps.md) — **G-dec** row: parse + policy + `MirFn.decorators`
...
| **7d-e (partial)** | Policy: reserved names, typosquat, `parallel_requires_disjoint` | **Done** — `decorator_exploits/` 4× `compile_fail` |
| **7d-b (partial)** | `@cpu` / `@vectorized` / `@parallel` on `def` | `@cpu` → `mir_cpu_def=1`; proc `@vectorized` → `ArraySimdScope` (`mir_vectorized_def_scope`); `@gpu`/`@parallel` MIR tags |
...
|-----|-------------|-----------|-----|
| **7d-b** | `@cpu` / `@parallel` / `@vectorized` on `def` lower to MIR | `mir_cpu_def=1`, `mir_vectorized_def_scope=1`, `mir_parallel_disjoint=1` | **G-dec** |
| **7d-c** | Proc `@parallel(disjoint=…)` inherits; scoped `@vectorized for` | `check-mir-parallel-decorator.sh` + `race_shared_memory` green | **G-dec**, **G-par** (policy only until **#387**) |
| **7d-d** | Stdlib + handbook gallery | `check-doc-provability-claims.sh` pass | **G-dec** (doc) |
| **7d-e** | `decorator def` expansion whitelist | `decorator_exploits/` all `compile_fail` on every PR | **G-dec** |

### G-par cross-link (7d-c)

Where elaboration relies on `parallel_for_disjoint_witness(stmt, proc_decorators)` string/heuristic checks in `policy_module.cpp`, document as **G-par Partial** and link **#387**.
...
- [ ] Close **#22** when acceptance boxes checked (after CI green)
