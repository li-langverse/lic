...
1. [provability-gaps.md](../../verification/provability-gaps.md) — **G-dec** row: parse + policy + `MirFn.decorators`; MIR elaboration slices landing
...
5. `scripts/check-mir-*-decorator.sh` — MIR telemetry + elaboration smokes
...
| Sub | Asset | Status |
|-----|-------|--------|
| **7d-a** | Parse `@` on `def`/`for`/`while` | **Done** |
| **7d-e (partial)** | Policy: reserved names, typosquat, `parallel_requires_disjoint` | **Done** — `decorator_exploits/` 4× `compile_fail` in CI |
| **7d-b (partial)** | `@cpu`/`@vectorized`/`@gpu`/`@parallel` on `def` | `@cpu` → `mir_cpu_def=1`; proc `@vectorized` → `ArraySimdScope` (`mir_vectorized_def_scope`); `@gpu`/`@parallel` MIR tags |
| **7d-c (partial)** | `@vectorized` on `for` → `ArraySimdScope` | **Done** (#150); proc `@parallel(disjoint=)` inherit → `mir_parallel_disjoint` |
...
|-----|-------------|-----------|-----|
| **7d-b** | `@cpu` / `@parallel` / `@vectorized` on `def` lower to MIR | `check-mir-cpu-decorator.sh`; `vectorized_def_scope_ok.li` → `mir_vectorized_def_scope=1` | **G-dec** |
| **7d-c** | Proc `@parallel(disjoint=…)` inherits; `@vectorized for` scoped | `parallel_def_disjoint_inherit.li` → `mir_parallel_disjoint=1`; `check-mir-parallel-decorator.sh` | **G-dec**, **G-par** (policy until **#387**) |
| **7d-d** | Stdlib + handbook gallery | `check-doc-provability-claims.sh` pass | **G-dec** (doc) |
| **7d-e** | `decorator def` whitelist + exploits | `decorator_exploits/` all `compile_fail` on every PR | **G-dec** |

### G-par cross-link (7d-c)

Where elaboration relies on `parallel_for_disjoint_witness(stmt, proc_decorators)` in `policy_module.cpp`, document as **G-par Partial** and link **#387** — do **not** mark **G-par Done** when only **G-dec** elaboration lands.
...
| MIR smokes | `check-mir-cpu-decorator.sh`, `check-mir-parallel-decorator.sh`, `check-mir-vectorized-decorator.sh` |
| Doc gate | `scripts/check-doc-provability-claims.sh` |

## Rollout

1. **lic PR (elaboration):** 7d-b + 7d-c — MIR lowering + decorator tests + MIR scripts + **G-dec** doc-c.
2. **lic PR (surface):** 7d-e — `decorator def` expansion whitelist.
3. Pair with **#387** Lean PR when proc-tag parity tests are ready.
4. Close **#22** when acceptance boxes checked (7d-e macro expansion may remain open).
