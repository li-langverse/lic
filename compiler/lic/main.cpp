...
int verify_file(const char* path, bool run_lean, bool strict_lean) {
  ...
  const std::size_t mir_omp_parallel_for = li::count_mir_omp_parallel_for(mir);
  ...
  std::cout << "verify: procs=" << vc.proc_count << " mir_fns=" << vc.mir_fn_count
            ...
            << " mir_omp_parallel_for=" << mir_omp_parallel_for
  ...
  if (li::terminal_color_enabled()) {
    ...
    li::print_verify_telemetry(std::cout, "mir_omp_parallel_for",
                               std::to_string(mir_omp_parallel_for));
