...
- **WP-SCI-03 Phase 2 slice** — `sim_scientific_oracle_checksum_cfd`, `sim_scientific_oracle_checksum_fea`, `sim_scientific_oracle_checksum_qm_integral`; split `run_algo_registry` dispatch for PDE rows 201–204 (heat), 205–211 (CFD), 212–216 (FEA), QM 401–404 (integral trace); algo 418 remains documented stub (`checksum=1.001`); extended `run_algo_registry_tier2.li` and `scientific_oracle_bench.li`.
...
- `run_md_lj_smoke` / `run_heat_smoke` use tier-2 oracle checksums (not scalar stub / constant 1.0); `li_sim_scientific_version` → 7 (WP-SCI-03 CFD/FEA/QM-integral dispatch).
