...
| **li-sim-scientific** | partial → tier-2 oracle | CPU + `@gpu` MD oracle smoke | Lib builds with open VC; smokes verify | **Real:** 4-particle LJ chain + 1D heat stencil + CFD Poiseuille + FEA bar oracles; `run_cfd_tier2_registry` (205–210), `run_fea_tier2_registry` (211–216). **Placeholder:** `run_algo_registry_stub` (checksum 1.001) for QM 401–432 | WP-SCI-03 partial (CFD/FEA); QM rows + LAMMPS/GROMACS external oracle remain |
