...
| **G-vc** | Partial | **PH-2e v1 gate green** ([#21](https://github.com/li-langverse/lic/issues/21)): typed `AutoVC.lean`; `lic verify` `witnessed_ensures=` + `mir_return_linked=` (`compiler/verify/vc_witness.cpp`); `vc_emit_contracts.sh`, `mir_vc_witness.sh`, `contracts_discharge_corpus.sh`, `check-autovc-open-goals.sh` on closed corpus. **Closed slice:** call-site `requires`, const-local discharge, E0303/E0304/E0305; `sqrt_open_bound` float `abs` bound (trusted libm). **Still open:** opaque `vec3_dot`-style returns; loop implementations vs closed-form `ensures` |
...
| **G-vc** | VC generation | Contracts → proof obligations | **Partial** — **PH-2e v1 gate green:** typed `AutoVC.lean`; `lic verify` witness telemetry (`vc_witness.cpp`); open-goals checker on closed corpus. **Closed slice:** call-site `requires`, const-local discharge, E0303/E0304/E0305; `sqrt_open_bound` trusted libm. **Open:** float opaque returns, loop vs closed-form `ensures` | **2e** | [phase-02e plan](../superpowers/plans/2026-05-14-phase-02e-contracts-refinements.md); `vc_emit_contracts.sh`, `mir_vc_witness.sh`, `contracts_discharge_corpus.sh`, `check-autovc-open-goals.sh`, `discharge_caller_requires_lean.sh`, `discharge_caller_requires_local_lean.sh`, `prove_reject/weak_ensures_true.li`, `./li-tests/run_all.sh contracts_verify` |
