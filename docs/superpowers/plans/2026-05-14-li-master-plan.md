...
- [ ] Phase 8p — Parallel compile + CI throughput — **partial (8p-a/c/d done; 8p-b shipped):** 8p-a parallel `run_all` + isolated `LI_BUILD_DIR` ([#186](https://github.com/li-langverse/lic/pull/186), [#200](https://github.com/li-langverse/lic/pull/200)); 8p-b parallel `lic-workspace-build.sh` pool ([#385](https://github.com/li-langverse/lic/issues/385)); 8p-c/d `ResourceOptions` + `lic build --jobs` reserved pass + CI test-job smokes; **open:** wall-time SLO measurements in baseline ([§ 8p](#phase-8p--parallel-compile--ci-throughput))
