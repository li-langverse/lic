...
- [ ] Phase 8p — Parallel compile + CI throughput — **partial (8p-a/c):** 8p-a parallel `run_all` + isolated `LI_BUILD_DIR` ([#186](https://github.com/li-langverse/lic/pull/186), [#200](https://github.com/li-langverse/lic/pull/200)); 8p-c `ResourceOptions` + `lic build --jobs` reserved pass + CI test-job smokes; **8p-b/d shipped:** workspace pool + `local-ci.sh` `wall_s` logging ([#385](https://github.com/li-langverse/lic/issues/385)); **open:** wall-time SLO measure + phase exit gate ([§ 8p](#phase-8p--parallel-compile--ci-throughput))
