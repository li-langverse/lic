...
if [[ -z "${BENCHMARKS_ROOT:-}" ]]; then
  ...
  # Lite fallback: vendored results/competitive only when no sibling harness exists.
  if [[ -z "${BENCHMARKS_ROOT:-}" ]]; then
    for _c in "$_lic/benchmarks" "${LI_LANGVERSE_ROOT:-}/benchmarks" "${LANGVERSE:-}/benchmarks"; do
