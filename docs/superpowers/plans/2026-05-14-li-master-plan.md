...
| 7d-b–e | Decorator elaborate + exploits | **G-dec**, **G-par** | `check-mir-*-decorator.sh` + `./li-tests/run_all.sh decorator_exploits decorators` green | language/decorators.md, fast-math guide, [provability-gaps](../../verification/provability-gaps.md#g-dec) |
...
- [ ] Phase 7d — Execution decorators — **partial (#22, #150):** `@cpu` / `@parallel(disjoint=…)` / `@vectorized(lanes=…)` / `@gpu` MIR proc tags + `check-mir-*-decorator.sh`; `@vectorized` on `for` → `ArraySimdScope`; **7d-e** `decorator_exploits` CI green; **open:** Lean **P-dec**, `@async` MIR, `decorator def` expansion — see **G-dec**, **G-par**
...
- **7d partial:** `@cpu` / `@parallel` / `@vectorized` / `@gpu` MIR proc tags + `decorator_exploits` CI (**#22**); structured `disjoint=` proofs still **G-par** open
...
| **7d** | **G-par**, **G-dec** | Lean **P-dec**; `@async` / `decorator def` expansion; structured `disjoint=` proofs (**G-par**) |
