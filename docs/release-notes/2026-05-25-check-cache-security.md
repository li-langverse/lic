...
2. Run `./scripts/build.sh && ./li-tests/cache_exploits/check_cache_exploits.sh`.
3. Next: merge after WP3 `feat/lic-check-workspace-cache`; integrator wires full `ci.sh` flag-only pool.
4. Blocked: none.
...
Stack PR on `feat/lic-check-workspace-cache` (WP3).
