...
rows = [
    ...
    },
    {
        ...
        "workload_note": "32x32 logical f32 via ArrayDesc -> ml_matmul_cpu_logical_32; run-only 50x mean cpu_sec",
        "executed": bool(liarray_matmul32.get("executed")),
        "li": li_row(liarray_matmul32, "tier3_cpu" if liarray_matmul32.get("executed") and liarray_matmul32.get("validity_gate_pass") else "pilot"),
        "competitors": [
            comp_row(numpy_m32, liarray_matmul32.get("cpu_sec"), "python_numpy", "NumPy BLAS matmul 32x32", "blas_labeled", "same logical 32x32 class"),
        ],
    },
    {
        "id": "matmul_lkir",
