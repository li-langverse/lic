...
MD_ORACLE="$BENCHMARKS_COMPETITIVE/md_oracle.toml"
if [[ ! -f "$MD_ORACLE" ]]; then
...
if harness.get("gate_script") != "scripts/ph-sci-md-oracle-competitive-gates.sh":
    errors.append("md_oracle: harness.gate_script must cite ph-sci-md-oracle-competitive-gates.sh")
if harness.get("li_tests") != "li-tests/tooling/md_external_oracle_stub.sh":
    errors.append("md_oracle: harness.li_tests must cite md_external_oracle_stub.sh")
for w in warnings:
    print(f"warn: {w}", file=sys.stderr)
for e in errors:
    print(f"error: {e}", file=sys.stderr)
