...
| **#574** (Lean witness) | Closed (`already_implemented`) | **PR [#902](https://github.com/li-langverse/lic/pull/902) still open** — `Discharge.lean` has no `broadcast_len1` on `main`; gap script still enforces absence |
...
| **#574** / **PR #902** | Lean witness for broadcast_len1 MIR (issue closed; merge pending — blocks `verify_ok` promotion on `main`) |
...
| **E** | **Downstream tracker honesty** — note #574/#526 closed vs open PRs | Comment on #618 linking #902/#909; do **not** claim Lean/NumPy-rank done until those PRs merge |
...
|-----|------|--------------|
| **G-math** | Partial → **Partial** (richer closed-slice bullet) | Add **closed compile slice:** length-1 element-wise broadcast (`broadcast_len1_*`, `compile_ok` only). **Still open on `main`:** Lean broadcast semantics (PR #902), NumPy-rank reject gate (PR #909), tier-1 red rows (#463) |
| **G-lean** | No change until PR #902 merges | `broadcast_len1_codegen_lean_gap.sh` still enforces no `Discharge.lean` broadcast |
| **G-math-syn** | No change | Syntax surface unchanged |

Do **not** promote manifest entries to `verify_ok` until **PR #902** merges to `main`.
...
4. Close **#618** when A–D exit gates pass; track Lean (#902) and NumPy-rank (#909) merges separately.
...
- [ ] Merge **PR #902** (Lean witness) and **PR #909** (NumPy-rank gate) before promoting PH-2i checkbox to done.
