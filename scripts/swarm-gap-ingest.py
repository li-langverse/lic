...
def ingest_verticals_stubs(gaps_by_id: dict[str, dict]) -> int:
    default_competitive = BENCHMARKS / "workloads" / "competitive"
    if not default_competitive.is_dir():
        default_competitive = BENCHMARKS / "competitive"
    vert = Path(os.environ.get("BENCHMARKS_COMPETITIVE", str(default_competitive))) / "verticals.toml"
    if not vert.is_file():
        return 0
