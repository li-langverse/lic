...
- [ ] Phase 7d — Execution decorators — **partial ([#22](https://github.com/li-langverse/lic/issues/22), [#387](https://github.com/li-langverse/lic/issues/387)):** proc `@cpu`/`@parallel(disjoint=)`/`@vectorized` MIR tags + `check-mir-*-decorator.sh`; proc `@vectorized` → body `ArraySimdScope` (`mir_vectorized_def_scope`); `@vectorized` on `for` scoped (#150); **open:** **7d-e** macro expansion, Lean **G-par** proofs — [PH-7d plan](2026-06-07-ph-7d-g-dec-mir-elaboration.md)
