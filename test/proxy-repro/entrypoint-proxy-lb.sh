...
RUNTIME=/proxy/httpd-lb.runtime.toml
...
sed "s/policy = \"ip_hash\"/policy = \"${POLICY}\"/" /proxy/httpd-lb.toml > "$RUNTIME"
python3 /opt/lic-scripts/validate-httpd-config.py "$RUNTIME" --allow-peer-host backend-a --allow-peer-host backend-b
python3 /opt/lic-scripts/setup-tls-httpd.py "$RUNTIME" --cert-dir /certs
python3 /opt/lic-scripts/flatten-httpd-config.py "$RUNTIME" -o "$CONF"
sed -i "s|^tls_cert_dir=.*|tls_cert_dir=/certs|" "$CONF"
grep -q "^upstream_balance=gitlab|${POLICY}" "$CONF"
