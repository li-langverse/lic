...
| **G-math** | Math / `A @ B` | Shape errors at compile time; no user `simd(...)` | **Partial** — **closed slice:** 9× `prove_lean_ok` linalg + `discharge_linalg_int_lean.sh`; `math_linalg/` compile tests incl. length-1 broadcast (`broadcast_len1_*`, `compile_ok` only); tier-1 `tier1_li_vs_cpp.sh`; Lean broadcast witness [#574](https://github.com/li-langverse/lic/issues/574) open | **2i**, **7e**, **2f** | `li-tests/math_linalg/`, `li-tests/math_linalg/broadcast_len1_*.li`, `li-tests/contracts_verify/linalg_*_closed.li`, `li-tests/tooling/discharge_linalg_int_lean.sh`, `li-tests/tooling/broadcast_len1_codegen_lean_gap.sh`, `li-tests/tooling/tier1_li_vs_cpp.sh` |
...
| `li-tests/math_linalg/` | **Partial** — 1d/2d `@`, element-wise, matmul compile tests; length-1 broadcast (`broadcast_len1_*`, `compile_ok`; Lean witness #574 open) (2i/7e) |
