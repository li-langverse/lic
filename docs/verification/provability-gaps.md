...
| **G-math** | Partial | **Closed slice (tier-1 smoke):** `tier1_li_vs_cpp.sh` + `li-tests/fixtures/tier1_math_perf_smoke.csv`. **Closed slice (tier-1 perf):** `matmul_naive`, `horner_pure_li` ≤1.2× C++ when `latest.csv` refreshed. **Closed slice:** 2×2 float `@` Prop; prelude `dot`/`norm`/`axpy`/**; `reductions/{sum_non_array,dot_len_mismatch}.li` compile_fail; loop-dot witness; P-linalg corpus |
...
| **G-math** | Math / `A @ B` | Shape errors at compile time; no user `simd(...)` | **Partial** — linalg corpus + `math_linalg/` + tier-1 reporter smoke (`tier1_math_perf_smoke.csv`) | **2i**, **7e**, **2f** | `li-tests/math_linalg/`, `li-tests/tooling/tier1_li_vs_cpp.sh`, `discharge_linalg_int_lean.sh` |
