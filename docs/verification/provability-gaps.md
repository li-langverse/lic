...
| **G-vc** | Partial | **2e v1 gate green** (see [phase-02e plan](../superpowers/plans/2026-05-14-phase-02e-contracts.md)): typed `AutoVC.lean`; `lic verify` `witnessed_ensures=` + `mir_return_linked=`; open-goals checker on closed corpus. **Closed slice:** `sqrt_open_bound` float `abs` bound (trusted libm axiom). Still open: opaque `vec3_dot`-style returns; loop implementations vs closed-form `ensures` |
...
| **G-vc** | VC generation | Contracts → proof obligations | **Partial** — **2e v1 gate:** typed AutoVC + MIR witness telemetry + open-goals checker; **closed slice:** call-site `requires`, const-local discharge, E0303/E0304/E0305; open: float `abs`, opaque returns | **2e** | [phase-02e plan](../superpowers/plans/2026-05-14-phase-02e-contracts.md); `vc_emit_contracts.sh`, `mir_vc_witness.sh`, `check-autovc-open-goals.sh`, `compiler/verify/vc_witness.cpp`, `discharge_caller_requires_lean.sh`, `discharge_caller_requires_local_lean.sh`, `contracts_discharge_corpus.sh`, `prove_reject/weak_ensures_true.li` |
