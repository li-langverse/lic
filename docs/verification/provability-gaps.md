...
| **G-math** | Partial | **Closed slice (correctness):** 1d `@` → `ArrayDotF64` SIMD gather; 2D `@` → `ArrayMatMul2DF64` IKJ+FMA; 512³ blocked → `ArrayMatMulBlocked2DF64` (BK=64). **Closed slice (tier-1 perf):** `simd_dot` ≤1.2× C++. **Open (tier-1 perf):** `matmul_blocked` (~1.26× local), `matmul_naive` / `horner_pure_li` on dashboard — see [PH-7e matmul Done criteria](../superpowers/plans/2026-06-07-ph7e-simd-matmul-done-criteria.md) · **#463**. **Closed slice (Lean):** 2×2 float `@` Prop (`linalg_mat2_at2_float_closed`); MIR preservation deferred (**#32**). **Closed slice (surface):** prelude `norm`/`axpy`/**, `math_linalg/`, loop-dot witness, P-linalg int corpus |
...
| **G-math** | Math / `A @ B` | Shape errors at compile time; no user `simd(...)` | **Partial** — **closed:** 1d `ArrayDotF64`, 2D `ArrayMatMul2DF64`, blocked `ArrayMatMulBlocked2DF64` codegen + `math_linalg/`; **open:** `matmul_blocked` ≤1.2× C++ (#463) | **2i**, **7e**, **2f** | `li-tests/math_linalg/`, `matmul_loop_codegen_witness_gap.sh`, `mat2_at2_mir_codegen_lean_gap.sh`, `sum_dot_product_equiv_gap.sh`, `discharge_linalg_int_lean.sh`, `scripts/check-tier1-li-vs-cpp.sh` (`simd_dot`, `matmul_naive`, `matmul_blocked`), [7e matmul Done criteria](../superpowers/plans/2026-06-07-ph7e-simd-matmul-done-criteria.md) |
