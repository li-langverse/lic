...
- [ ] Phase 7d — Execution decorators — **partial ([#22](https://github.com/li-langverse/lic/issues/22) 7d-b/c):** `@cpu`/`@gpu`/`@parallel`/`@vectorized` MIR proc tags + telemetry; `@vectorized` on `for`/`def` → `ArraySimdScope`; **def `@parallel(disjoint=)`** inherits to nested `parallel for`; **open:** 7d-e macro expansion, Lean **G-par** proofs ([#387](https://github.com/li-langverse/lic/issues/387))
