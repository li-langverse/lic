...
| **Parallel disjointness** | Lean + structured proofs | **Partial** — policy witnesses in `Discharge.lean`; full iteration-independence Props open |
| **Index bounds (release)** | Refinement / proved → no user traps | **Partial** — MIR/runtime paths still evolving |
| **Decorators (`@parallel`, …)** | Compile-time elaboration + proofs | **Partial** — MIR proc tags on `def` (`mir_parallel_disjoint=`, `mir_vectorized_proc=`); Lean **P-dec** open |
...
| **G-dec** | Partial | **Closed slice:** MIR proc tags + `check-mir-*-decorator.sh` + `parallel_disjoint_lean_opaque_gap.sh`; Lean **P-dec** open |
...
| **G-dec** | Execution decorators | Static elaboration; reserved names; no runtime | **Partial** — **closed slice:** 4× `decorator_exploits` `compile_fail`; `@vectorized`/`@parallel` MIR proc tags on `def`; `@vectorized` on `for` (`ArraySimdScope`) | **7d** | `contracts_discharge_corpus.sh`, `check-mir-{parallel,vectorized}-decorator.sh`, `decorator_exploits/` |
