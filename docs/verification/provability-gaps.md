...
| **G-math** | Partial | **Tier-1 perf (advisory, [#463](https://github.com/li-langverse/lic/issues/463)):** dashboard green for six formerly-red ids (`matmul_*`, ML trio, `num_gmres`; ingest 2026-06-01, `ecosystem-audit.json` red `[]`). Local `./scripts/check-tier1-li-vs-cpp.sh` on `tier-tier1.csv` (2026-06-07): `simd_dot` 0.82×, `matmul_naive` 1.17×, `matmul_blocked` 0.36×, `horner_pure_li` 0.65× — advisory cap 1.2×; strict via `LI_TIER1_PERF_STRICT=1`. Do **not** claim tier-1 “closed slice” without measured ratio + ingest date. **Closed slice (proof):** full 2×2 float `@` Lean Prop (`linalg_mat2_at2_float_closed`, `mat2_at2_float_spec`). **Closed slice:** `linalg_dot4_float_closed` (prelude `dot`), `linalg_mat2_callproc_float_closed`, prelude `norm`/`axpy`/**, `math_linalg/reductions/`, loop-dot witness, P-linalg corpus. **Open:** full float `@` Props; re-verify after codegen changes |
...
| **G-math** | Math / `A @ B` | Shape errors at compile time; no user `simd(...)` | **Partial** — **closed slice (proof):** 9× `prove_lean_ok` linalg + `discharge_linalg_int_lean.sh`; `math_linalg/` compile tests. **Tier-1 perf:** advisory via `check-tier1-li-vs-cpp.sh` + dashboard ingest ([#463](https://github.com/li-langverse/lic/issues/463)); do not overclaim vs measured ratios | **2i**, **7e**, **2f** | `li-tests/math_linalg/`, `li-tests/contracts_verify/linalg_*_closed.li`, `li-tests/tooling/discharge_linalg_int_lean.sh`, `li-tests/tooling/tier1_li_vs_cpp.sh` |
