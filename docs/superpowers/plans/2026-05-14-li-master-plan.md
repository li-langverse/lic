...
- [ ] Phase 7d — Execution decorators — **partial ([#22](https://github.com/li-langverse/lic/issues/22)):** `@vectorized` on `for` → `ArraySimdScope`; **7d-b/c closed slice:** `@cpu`/`@parallel`/`@vectorized` MIR proc tags + `check-mir-*-decorator.sh`; proc `@parallel(disjoint=)` inherits to nested `parallel for` (policy); **open:** 7d-e macro expansion, Lean **G-par** proofs ([#387](https://github.com/li-langverse/lic/issues/387)) — [PH-7d plan](../superpowers/plans/2026-06-07-ph-7d-g-dec-mir-elaboration.md)
