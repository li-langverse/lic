...
## 7. Deliverables to add (plan maintenance)
...
## 8. Package taxonomy (avoid studio-only trap)
...
**Maintainers:** Update this file when adding a vertical to World Studio or a new `benchmarks/competitive/*.toml` slice. Bump `updated` in registry meta when running quarterly SOTA review.
