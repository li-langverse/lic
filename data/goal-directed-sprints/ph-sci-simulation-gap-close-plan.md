...
| **li-sim-scientific** | partial → tier-2 oracle | CPU + `@gpu` MD oracle smoke | Lib builds with open VC; smokes verify | **Real:** 4-particle LJ chain + 1D heat stencil oracles, `run_multi_physics_at_step`, QM 401–432 tier-2 (`chem_dft_scf_iteration_scaffold`). **Placeholder:** `run_algo_registry_stub` (checksum 1.001) for undocumented IDs only | WP-SCI-03 partial (QM landed); CFD/FEA use heat oracle; no LAMMPS/GROMACS external oracle |
