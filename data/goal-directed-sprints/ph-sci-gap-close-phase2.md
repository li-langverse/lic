...
**WP-SCI-03 partial:** QM ids 401–432 tier-2 dispatch landed (`run_qm_tier2_registry`); CFD/FEA rows remain for WP-SCI-05/06.  
...
1. **WP-SCI-03** — `run_algo_registry` real kernels (CFD/FEA/QM rows); extend `run_algo_registry_tier2.li`. **Partial (2026-06-08):** QM registry 401–432 tier-2 via `sim_scientific_oracle_checksum_qm` (`chem_dft_energy_kernel_hartree`); CFD/FEA rows remain stub (WP-SCI-05/06).
