#!/usr/bin/env bash
# G-math / PH-2i: length-1 broadcast lowers in MIR/codegen AND has Lean semantics + VC witness.
set -euo pipefail
...
ADD_CLOSED="$ROOT/li-tests/contracts_verify/linalg_broadcast_len1_add_float4_closed.li"
MUL_CLOSED="$ROOT/li-tests/contracts_verify/linalg_broadcast_len1_mul_int4_closed.li"
...
if ! grep -q 'broadcast_len1_mul_int4_spec' "$DISCHARGE"; then
  echo "FAIL: Discharge.lean should define broadcast_len1_mul_int4_spec" >&2
  exit 1
fi
if ! grep -q 'witness_broadcast_len1_spec' "$WITNESS"; then
...
rm -f "$AUTOVC"
"$LIC" build "$ADD_CLOSED" -o /dev/null 2>/dev/null
...
if ! grep -q 'broadcast_len1_add_float4_spec_proved' "$AUTOVC"; then
  echo "FAIL: AutoVC should discharge broadcast_len1_add via Discharge theorem" >&2
  exit 1
fi

rm -f "$AUTOVC"
"$LIC" build "$MUL_CLOSED" -o /dev/null 2>/dev/null
if ! grep -qi 'broadcast_len1_mul_int4_spec' "$AUTOVC"; then
  echo "FAIL: AutoVC should reference broadcast_len1_mul_int4_spec" >&2
  exit 1
fi
...
echo "PASS broadcast_len1_codegen_lean_gap: MIR/codegen + Lean broadcast_len1 discharge closed"
