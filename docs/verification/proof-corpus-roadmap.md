...
| `sqrt_open_bound.li` | `abs(result² - x) < ε` with `li_rt_sqrt` body | **Closed** — `prove_lean_ok` + `discharge_sqrt_open_lean.sh` |
...
| `contracts_discharge_corpus.sh` | **ok** | Trivial/const/index/caller-requires/**linalg closed** + `discharge_sqrt_open_lean.sh` |
...
| **P-float** | `Float.abs`, sqrt error bounds | **Partial** — `sqrt_open_bound` corpus closed (`sqrt_open_bound_spec_proved`); float `abs`/ULP policy open | `sqrt_open_bound.li` + `discharge_sqrt_open_lean.sh` |
