...
- [ ] Phase 2i — Math / linalg surface — **partial:** **2i-a/c** (#148); **2i-b** `norm`, `sum`/`dot`, `reductions/` (+ `sum_non_array.li` compile_fail); same-length `**`, prelude `axpy`, scalar×array; **length-1 broadcast** pass (`broadcast_len1_*.li`); general rank broadcast open; float `@` Props closed (`mat2_at2_eval`)
...
| **2i / 7e** | **G-math** | general rank broadcast, loop-dot proof, remaining tier-1 strict rows |
