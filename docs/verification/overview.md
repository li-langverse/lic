...
!!! important "Current gaps"
    **`lic build` does not yet run Lean 4** or full VC discharge. Parallel disjointness uses **heuristic** policy checks; decorator MIR proc tags are wired (**G-dec** partial) but Lean **P-dec** proofs remain open. See **[Provability gaps (today)](provability-gaps.md)** and the **[proof database](proof-database/README.md)** before claiming a full proof certificate.
