...
- **Remaining lib gaps (Phase 2):** `run_algo_registry_stub` for CFD/FEA rows (501+ drug/auto families still stub); QM 401–432 now tier-2 via `chem_dft_scf_iteration_scaffold` (2026-06-08); `li-sim-viz` compose-only; scene `native_pixels` stub; particles MD force accumulation still weak.
...
- **Status:** partial (2026-06-08) — QM family 401–432 tier-2 via `run_qm_tier2_registry` + `sim_scientific_oracle_checksum_qm_dft`; CFD/FEA rows remain stub until WP-SCI-05/06.
