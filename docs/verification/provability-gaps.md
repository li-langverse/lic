...
| **G-par** | Partial | **Closed slice:** `parallel for` disjoint → `OmpParallelFor.parallel_disjoint_proven` + `check-mir-parallel-for-disjoint.sh`; `disjoint_par_policy_witness` (**P-par** partial). **Open:** iteration-independence Lean specs |
