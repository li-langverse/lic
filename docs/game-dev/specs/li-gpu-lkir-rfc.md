# RFC: li-gpu, LKIR (deprecated stub)

**Status:** Redirect — use [lig-rfc.md](../../game-dev/specs/lig-rfc.md)

The **`li-gpu`** package and **`import gpu`** surface are renamed to **`lig`** (`import lig`) under PH-HW WP1.

| Former doc | Canonical doc |
|------------|----------------|
| li-gpu-lkir-rfc.md (this file) | [lig-rfc.md](../../game-dev/specs/lig-rfc.md) |
| `packages/li-gpu` | `packages/lig` (see `git mv` in lig-rfc.md) |
| Kernel catalog (TBD) | [lig-kernel-catalog.md](../../game-dev/lig-kernel-catalog.md) |

Do not add new content here; edit **lig-rfc.md** only. Li syntax policy (`def` only in new docs, no bare `proc`) lives in **lig-rfc.md** and the game-dev RFC index.
