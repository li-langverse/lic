...
LIC="${LIC:-}"
# Prefer native builds; on native Linux never execute build-wsl (glibc mismatch risk).
...
[[ -x "$LIC" ]] || { echo "ph-ml-wave12-gates: build lic (./scripts/build.sh or build-wsl in WSL)"; exit 1; }
