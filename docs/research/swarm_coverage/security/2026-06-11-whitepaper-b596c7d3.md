...
1. Scorecard: `ecosystem-quality-report.json` @ 00:26Z → grade C (75.6), `unattended_safe: true`
2. Briefing: `agent-briefing.json` @ 01:26Z — recommends `ci_maintainer`, `security_auditor`; heap schedules `ci_maintainer` only
3. CWE feed: `security-cwe-feed.json` @ 01:26Z vs `lic/security/cve-catalog.json`
