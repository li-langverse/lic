...
- **sim-p2-qm-dft-scf (418)** — `sim_scientific_oracle_checksum_qm_dft_scf`, `run_qm_dft_scf_tier2_registry`, `algo_qm_dft_scf_energy()` dispatch; H₂ STO-3G SCF oracle via `echem_dft_h2_energy_hartree()`; smokes `qm_dft_scf_interface_smoke.li`, composable `import_chem_dft_smoke.li` (lic#478).
