...
- [ ] Phase 2i — Math / linalg surface — **partial:** **2i-a/c** (#148); **2i-b** `norm`, `sum`/`dot`, `reductions/`, same-length `**`, prelude `axpy`, scalar×array; **length-1 broadcast** (`array[1]`→`array[N]` element-wise); full NumPy rank broadcast still open; float `@` Lean Props closed (`mat2_at2_eval` semantic); MIR `ArrayMatMul2DF64` ↔ Lean bridge **deferred** (**G-math**, Phase 7e)
...
- [x] Phase 2e — Contracts + refinements — **merged (PR #83):** call-site `requires` (**E0304**), refinement types (**E0305**), if-guard VC discharge, import/extern; corpus [proof-corpus-roadmap.md](../verification/proof-corpus-roadmap.md); **P-float** discharge path closed (`sqrt_open_bound_spec_proved`); opaque float `vec3_dot` / loop vs closed-form `ensures` still open (**G-vc**)
- [x] Phase 2f — Lean 4 verify — **partial (#83, #151, #155):** default `lake build AutoVC` on `lic build`; **P-linalg** closed corpus + loop dot (`dot4_int_loop_eval_spec`); fib/recursive call-site + `decreases`/`_par*` VCs typecheck; **P-float** discharge lemma closed; corpus specimen `sqrt_open_bound.li` intentional open (`verify_open_ok`, BUG-C-12); **G-lean** / **G-vc** remain **Partial** — [still open gaps](../verification/provability-gaps.md#still-open-report-every-session)
...
- [ ] Phase 7e — Math → SIMD/parallel lowering — **partial (#148, #150, #155):** loop matmul + FMA horner; tier-1 advisory ≤1.2× (`matmul_naive`, `horner_pure_li`); **`check-tier1-li-vs-cpp.sh`** strict optional; **open:** remaining tier-1 slices, general `A @ B` SIMD matmul MIR lowering + Lean↔`ArrayMatMul2DF64` preservation (**G-math** deferred; `mat2_at2_mir_codegen_lean_gap.sh`)
