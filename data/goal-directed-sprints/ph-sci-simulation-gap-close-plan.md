...
| **li-sim-scientific** | partial → tier-2 oracle | CPU + `@gpu` MD oracle smoke | Lib builds with open VC; smokes verify | **Real:** MD/heat/CFD/FEA/QM tier-2 oracles + `run_algo_registry` dispatch (205–216, 401–432). **Placeholder:** `run_algo_registry_stub` (checksum 1.001) for 501–903 families only | WP-PLAT-05 external MD oracle |
...
#### WP-SCI-03 — `run_algo_registry` real kernels (extends existing ID) — **LANDED** (tier-2 slice, 2026-06-08)
...
- **Acceptance:** `run_algo_registry_tier2.li` extended; stub only for explicitly documented IDs (501–903).
...
| Phase 2 | 11 | **OPEN** (WP-SCI-03 landed) | WP-SCI-04 / WP-PLAT-05 |
