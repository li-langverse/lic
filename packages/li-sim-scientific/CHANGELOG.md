...
- **WP-SCI-03 CFD/FEA/QM tier-2** — `sim_scientific_oracle_checksum_cfd`, `_fea`, `_qm_overlap`, `_qm_dft`; `run_cfd_tier2_registry` (205–210), `run_fea_tier2_registry` (211–216), `run_qm_integral_tier2_registry` (401–404), `run_qm_dft_tier2_registry` (405–432); `vertical_cfd_lid_driven_cavity`, `vertical_fea_linear_elasticity`; smokes `run_algo_registry_tier2.li` + `run_algo_registry_tier2_cfd_fea_qm.li`.

### Changed

- `run_md_lj_smoke` / `run_heat_smoke` use tier-2 oracle checksums (not scalar stub / constant 1.0); `li_sim_scientific_version` → 7 (WP-SCI-03 CFD/FEA/QM).
- **WP-SCI-03 echem smoke alignment** — echem interface smokes expect `li_sim_scientific_version` 7; AIMD oracle checksum floor `1.0e-8` when energy drift is exactly zero; host-float smoke thresholds use `< 1.0e-12` / `< 2.0` bounds; CFD/FEA/QM tier-2 split to `run_algo_registry_tier2_cfd_fea_qm.li`.
