...
**Agent verification (2026-06-04):** run `code_implementer-1780603414676` — rebased PR [#760](https://github.com/li-langverse/lic/pull/760) onto `main`; wave gates use `lic-bin-select.sh`, program-complete uses `lic-runnable.sh` with `LIC_ROOT` fallback (stale `build-wsl` lic required GLIBC 2.38 on glibc 2.36 agents).
