...
`li_rt_sqrt` accuracy is axiomatized here (not proved from IEEE). Migrate to `trusted.lean` when the
Trusted lake target is wired. See **G-hw** in `docs/verification/provability-gaps.md`.
-/
