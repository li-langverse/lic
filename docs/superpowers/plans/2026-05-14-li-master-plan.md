...
- [ ] Phase 2i — Math / linalg surface — **partial:** **2i-a/c** (#148); **2i-b** `norm`, `sum`/`dot`, `reductions/`, same-length `**`, prelude `axpy`, scalar×array; **length-1 broadcast done** (`broadcast_len1_*.li`); **full NumPy rank broadcast deferred** ([#526](https://github.com/li-langverse/lic/issues/526), reject gate + `broadcast_invalid_2x3_vs_2x1_float.li`); float `@` Props closed (`mat2_at2_eval`)
