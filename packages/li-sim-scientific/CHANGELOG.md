...
- **WP-SCI-03 Phase 2 CFD/FEA tier-2** — `sim_scientific_oracle_checksum_cfd`, `sim_scientific_oracle_checksum_fea`, `run_cfd_tier2_registry`, `run_fea_tier2_registry`; split PDE registry dispatch (201–204 heat, 205–210 CFD, 211–216 FEA); `vertical_cfd_lid_driven_cavity`, `vertical_fea_linear_elasticity`; extended `run_algo_registry_tier2.li`.
...
- `run_md_lj_smoke` / `run_heat_smoke` use tier-2 oracle checksums (not scalar stub / constant 1.0); `li_sim_scientific_version` → 7 (WP-SCI-03 CFD/FEA tier-2).
