...
- **WP-SCI-03 CFD/FEA/QM tier-2 kernels** — `sim_scientific_oracle_checksum_cfd`, `sim_scientific_oracle_checksum_fea`, `sim_scientific_oracle_checksum_qm`; `run_cfd_tier2_registry` (205–210), `run_fea_tier2_registry` (211–216), `run_qm_tier2_registry` (401–432); `vertical_cfd_lid_driven_cavity` / `vertical_fea_linear_elasticity` in `li-sim`; extended `run_algo_registry_tier2.li`.
...
- `run_heat_tier2_registry` scoped to PDE heat rows 201–204 (CFD/FEA split to dedicated oracles); `li_sim_scientific_version` → 7 (WP-SCI-03 CFD/FEA/QM).
