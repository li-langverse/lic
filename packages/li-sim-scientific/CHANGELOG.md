...
- **WP-SCI-03 QM tier-2 registry** — `sim_scientific_oracle_checksum_qm_dft`, `run_qm_tier2_registry` for algo ids 401–432 (incl. `algo_qm_dft_scf_energy` 418); `run_algo_registry_tier2.li` extended; stub retained only for documented non-QM rows (501+, 601+, …).
...
- `run_md_lj_smoke` / `run_heat_smoke` use tier-2 oracle checksums (not scalar stub / constant 1.0); explicit array zero-init in MD/heat oracles; `li_sim_scientific_version` → 7 (WP-SCI-03 QM).
