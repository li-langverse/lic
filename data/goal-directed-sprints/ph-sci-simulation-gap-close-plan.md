...
| **li-sim-scientific** | partial → tier-2 oracle | CPU + `@gpu` MD oracle smoke | Lib builds with open VC; smokes verify | **Real:** 4-particle LJ chain + 1D heat stencil + CFD/FEA tier-2 oracles (511–514, 521–523), `run_multi_physics_at_step`. **Placeholder:** `run_algo_registry_stub` (checksum 1.001) for QM 401–432 | WP-SCI-03 partial (CFD/FEA landed); QM rows + LAMMPS/GROMACS external oracle remain |
