...
- **PH-ML Wave 4d — hardware smoke and CI paths:** lavapipe workflow, CUDA smoke doc, `@gpu` on `for` compile_fail, WP-RL-04 fork issue template — [2026-05-29-ph-ml-wave-4d.md](docs/release-notes/2026-05-29-ph-ml-wave-4d.md).
- **PH-HW LKIR file parser (WP-HW-04/05):** `li_rt_lkir_parse.c`, `lkir-validate.sh`, `lkir_file_validate.li`; bench parity runs matmul binary; tier3 MLP oracle in bench JSON; `linux-gpu-smoke.sh`.
- **PH-HW Metal device pilot (WP-HW-11, macOS):** `li_rt_lig_metal.mm` 2×2 matmul via runtime MSL; `LIG_EMIT_METAL=1`; `macos-metal-smoke.sh` — [metal-macos-smoke.md](docs/ci/metal-macos-smoke.md).
- **PH-HW WP-HW-09 PTX catalog:** `runtime/lig-ptx-catalog.toml`, `lig_cuda_ptx_catalog.c`, `check-lig-ptx-catalog.sh`; cuda-home-probe `ready_emit_cpu_ref` + blocked rows — [2026-05-29-ph-hw-wp09-ptx-catalog.md](docs/release-notes/2026-05-29-ph-hw-wp09-ptx-catalog.md).
- **PH-HW CUDA device pilot (WP-HW-08/09):** `li_rt_lig_cuda.c` + embedded PTX 2×2 matmul on RTX via `libcuda`; honest `gpu_timing_ns` in bench JSON; Vulkan `VkInstance` loader smoke (WP-HW-07 partial).
- **PH-HW WP-HW-08 hardening:** `cuCtxSynchronize`-bounded device timing; probe rejects non-positive ns — [2026-05-29-ph-hw-wp08-cuda-matmul-hardening.md](docs/release-notes/2026-05-29-ph-hw-wp08-cuda-matmul-hardening.md).
- **PH-HW WP-HW-08 LKIR dispatch:** `kid=1` gates on `matmul_f32.lkir` before CUDA device path — [2026-05-29-ph-hw-wp08-lkir-cuda-dispatch.md](docs/release-notes/2026-05-29-ph-hw-wp08-lkir-cuda-dispatch.md).
- **PH-HW WP-HW-12 MLP vendor fallback:** kid=2 CPU ref after LKIR gate; CUDA/Vulkan paths never fake `gpu_timing_ns` via matmul device — [2026-05-29-ph-hw-cuda-12-mlp-fallback.md](docs/release-notes/2026-05-29-ph-hw-cuda-12-mlp-fallback.md).
- **PH-HW platform split + NVIDIA lab CUDA:** `docs/ci/hardware-platform-matrix.md`, `metal-macos-smoke.md`, `detect-cuda-home.sh`; lab `CUDA_HOME=/usr/lib/cuda` + nvcc 12.4; WP-HW-09 partial — [2026-05-29-ph-hw-platform-cuda-lab.md](docs/release-notes/2026-05-29-ph-hw-platform-cuda-lab.md).
- **PH-ML Wave 5b — hardware probes:** CUDA toolkit doc, CPU matmul ref on `LIG_EMIT_CUDA=1`, lavapipe CI workflow, 8-env RL serial batch, GPU compile_fail corpus — [2026-05-29-ph-ml-wave-5b.md](docs/release-notes/2026-05-29-ph-ml-wave-5b.md).
