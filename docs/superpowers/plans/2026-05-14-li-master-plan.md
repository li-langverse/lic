...
- [ ] **Vision-LLM** — LLM-first + agent JSON diagnostics — **partial:** `lic check --format=json`, `lic diagnose`, `diagnostic-v1` schema, handover docs, manifest stub ([llm-first spec](../specs/2026-05-16-li-llm-first-design.md), [agent-handover](../ecosystem/agent-handover-formats.md), [competitive-safety-matrix](../ecosystem/competitive-safety-matrix.md) agent column)
- [x] **G-meta positioning** — one-page competitive safety matrix Carbon/Mojo/Rust vs Li ([#65](https://github.com/li-langverse/lic/issues/65), [competitive-safety-matrix](../ecosystem/competitive-safety-matrix.md); does **not** close **G-meta** proof gap)
