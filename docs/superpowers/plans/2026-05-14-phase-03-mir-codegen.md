...
### Phase 3 exit gate (**G-bnd**)

- [x] Native binary from minimal proc
- [x] `-O2` flag forwarded to clang in `--release` (C++: `-O3 -march=native` in `compile.cpp` when `--release`)
- [x] Bounds check calls present in IR for dynamic index (**G-bnd** partial — `li_bounds_fail` on dynamic index; proved literals skip runtime check)
