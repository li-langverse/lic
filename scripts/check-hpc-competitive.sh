...
required = ("id", "incumbent", "csv_lang", "status", "driver", "last_reviewed")
seen: set[str] = set()
for i, row in enumerate(oracles or []):
    if not isinstance(row, dict):
        errors.append(f"oracle[{i}]: not a table")
        continue
    for key in required:
        if key not in row:
            errors.append(f"{row.get('id', i)}: missing field {key}")
    oid = row.get("id", "")
    if oid in seen:
    ...
    driver = Path(str(row.get("driver", "")))
