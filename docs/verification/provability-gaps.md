...
| **G-vc** | Partial | **Phase 2e closed slice (CI):** `./scripts/check-phase-2e-gates.sh` — typed AutoVC (`vc_emit_contracts.sh`), MIR witnesses (`mir_vc_witness.sh`, `vc_witness.cpp`), corpus discharge + open-goal checker (`contracts_discharge_corpus.sh`, `check-autovc-open-goals.sh`). **Still open:** opaque `vec3_dot`-style returns; loop implementations vs closed-form `ensures` (Phase **2f**) |
...
| **G-vc** | VC generation | Contracts → proof obligations | **Partial** — **Phase 2e exit gates green:** `check-phase-2e-gates.sh` (typed AutoVC, MIR linkage, closed corpus + open-goal checker, E0303/E0304/E0305); open: opaque float returns, loop vs spec ensures | **2e** | `check-phase-2e-gates.sh`, `vc_emit_contracts.sh`, `mir_vc_witness.sh`, `compiler/verify/vc_witness.cpp`, `contracts_discharge_corpus.sh`, `check-autovc-open-goals.sh`, [phase-02e plan](../superpowers/plans/2026-05-14-phase-02e-contracts-vc.md) |
