...
**Last updated:** 2026-06-09 · **main @** `f3ea736e0` (Phase 0/1 + echem merged; start from latest `main`)  
...
1. **WP-SCI-03** — `run_algo_registry` real kernels (CFD/FEA/QM rows); extend `run_algo_registry_tier2.li`. **Partial landed** — branch `chore/agent-code_implementer-wp-sci-03-phase2-1781008035` (CFD lid-cavity oracle + FEA/QM tier-2 checksum witnesses; MD/heat/echem unchanged). **Partial landed** — CFD/FEA/QM tier-2 oracles + registry dispatch (`chore/agent-code_implementer-wp-sci-03-phase2-*`); MD external oracle column still WP-PLAT-05.
