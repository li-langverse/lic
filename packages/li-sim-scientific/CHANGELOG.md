...
- **WP-SCI-03 Phase 2 — QM tier-2 dispatch** — `sim_scientific_oracle_checksum_qm_dft`, `run_qm_tier2_registry`, CFD/FEA registry split (`run_cfd_tier2_registry`, `run_fea_tier2_registry`); `run_algo_registry` routes algo ids 401–432 through H2 STO-3G SCF oracle (not stub 1.001); smokes extended in `run_algo_registry_tier2.li`, `scientific_oracle_bench.li`.
...
- `run_md_lj_smoke` / `run_heat_smoke` use tier-2 oracle checksums (not scalar stub / constant 1.0); `li_sim_scientific_version` → 7 (WP-SCI-03 Phase 2 QM).
