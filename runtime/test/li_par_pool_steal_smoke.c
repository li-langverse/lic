...
int main(void) {
  ...
  li_par_pool_set_schedule(LI_PAR_SCHED_STEAL);
  li_par_pool_set_chunk_size(7);
  li_parallel_for_i64(0, 64, mark_body, 4);

  if (g_count != 64) {
    fprintf(stderr, "li_par_pool_steal_smoke: expected 64 iterations, got %d\n", g_count);
    return 1;
  }
  for (int i = 0; i < 64; ++i) {
    if (!g_seen[i]) {
      fprintf(stderr, "li_par_pool_steal_smoke: missing index %d\n", i);
      return 1;
    }
  }
  ...
  printf("li_par_pool_steal_smoke: ok\n");
  return 0;
}
