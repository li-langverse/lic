...
1. Scorecard: `ecosystem-quality-report.json` @ 11:29Z → grade C (76.1), `unattended_safe: true`
2. Briefing: `agent-briefing.json` @ 11:29Z — recommends `ci_maintainer`, `security_auditor`; heap schedules `ci_maintainer` only
3. CWE feed: `security-cwe-feed.json` @ 11:29Z vs `lic/security/cve-catalog.json`
4. Gap registry: 62 open rows; security-research plan_debt subset
5. Dry-run: `swarm-gap-ingest.py` / `swarm-gap-apply-actions.py` (PyYAML failure this pass)
