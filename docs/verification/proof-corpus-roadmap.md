...
| **P-par** | `parallel for` disjointness | **Partial** — `_par*` → `disjoint_*_spec` + policy witnesses + MIR tag + `iteration_independent_tile_spec` + `memory_disjoint_rows_spec` compositional slices | Full array aliasing Lean specs (7d-c) |
