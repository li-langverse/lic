...
| **li-sim-scientific** | partial → tier-2 oracle | CPU + `@gpu` MD oracle smoke | Lib builds with open VC; smokes verify | **Real:** 4-particle LJ chain + 1D heat stencil + CFD lid-proxy + FEA bar + QM SCF oracles; `run_multi_physics_at_step`. **Placeholder:** `run_algo_registry_stub` (checksum 1.001) for drug/bio/am/viz/auto rows only | WP-SCI-03 partial (CFD/FEA/QM tier-2 landed); no LAMMPS/GROMACS external oracle |
