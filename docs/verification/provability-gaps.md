...
| **G-par** | Partial | AST `policy_module` rejects missing disjoint, false `disjoint_row`, mut capture, borrow-in-par; **proc-level** `@parallel(disjoint=)` inherits to nested `parallel for` (`parallel_def_disjoint_inherit.li`); Lean `disjoint_elem` / iteration-independence proofs open (7d-c) |
