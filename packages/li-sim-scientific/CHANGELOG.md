...
- **sim-p2-qm-dft-scf (418)** — `sim_scientific_oracle_checksum_qm_dft_scf`, `run_qm_dft_scf_tier2_registry`, `algo_qm_dft_scf_energy()` dispatch; H₂ STO-3G SCF oracle via `qm_scf_h2_stub_energy()` (`physics.quantum` T0); smokes `qm_dft_scf_interface_smoke.li`, composable `import_chem_dft_smoke.li` (lic#478).
...
- `run_md_lj_smoke` / `run_heat_smoke` use tier-2 oracle checksums (not scalar stub / constant 1.0); `li_sim_scientific_version` → 7 (sim-p2-qm-dft-scf).
