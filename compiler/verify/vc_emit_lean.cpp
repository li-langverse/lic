...
void emit_contract_def(std::ostream& out, const Module& module, const ProcDecl& proc,
  ...
  if (c.kind == ContractKind::Ensures && c.expr) {
    if (ctx.proc != nullptr && witness_mat2_int_at2_spec(*ctx.proc, *c.expr)) {
      ...
      mat2_discharge_theorem = true;
               ...
               witness_broadcast_rhs_len1_add_float4_spec(*ctx.proc, *c.expr)) {
  ...
  if (par_policy && c.expr) {
    if (auto lean = expr_to_lean(*c.expr, ctx)) {
      ...
      prop = "True";
             ...
             !dot4_discharge_theorem && !broadcast_len1_discharge_theorem &&
    ...
    prop = "Li.Discharge.broadcast_rhs_len1_add_float4_spec";
    for (const auto& p : proc.params) {
      prop += ' ';
      prop += p.name;
    }
    prop += " (Li.Discharge.broadcast_rhs_len1_add_float4_eval";
    for (const auto& p : proc.params) {
      prop += ' ';
      prop += p.name;
    }
  ...
  const bool semantic_ensures =
      ((mat2_discharge_theorem || sqrt_discharge_theorem || dot4_discharge_theorem ||
        broadcast_len1_discharge_theorem || vec3_len_sq_discharge_theorem ||
        vec3_len_discharge_theorem) ||
  ...
  if (mat2_discharge_theorem && c.kind == ContractKind::Ensures) {
    ...
    out << "/-! Phase 2f: PH-2i length-1 broadcast — Li.Discharge.broadcast_rhs_len1_add_float4_spec (G-math) -/\n";
    out << "theorem " << name << "_proved";
    emit_formals(false);
    out << " : " << name;
    emit_args(false);
    out << " := Li.Discharge.broadcast_rhs_len1_add_float4_spec_proved";
    for (const auto& p : proc.params) {
      out << ' ' << lean_ident(p.name);
    }
    out << '\n';
