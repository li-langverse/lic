...
- **sim-p2 qm_dft_scf_energy (418)** — `sim_scientific_oracle_checksum_qm_dft_scf`, `run_qm_dft_scf_tier2_registry`, H₂ STO-3G SCF via `echem_dft_h2_energy_hartree()`; smokes `qm_dft_scf_interface_smoke.li`, composable `import_chem_dft_smoke.li`; PySCF H₂ oracle `benchmarks/competitive/pyscf_sto3g_h2_energy.py`.
