...
- **WP-SCI-03 Phase 2 CFD/FEA tier-2 kernels** — `sim_scientific_oracle_checksum_cfd`, `sim_scientific_oracle_checksum_fea`, `run_cfd_tier2_registry` (205–210), `run_fea_tier2_registry` (211–216), `vertical_cfd_lid_driven_cavity` / `vertical_fea_linear_elasticity`; `run_algo_registry_tier2.li` extended for algo 201/206/214 dispatch.
...
- `run_md_lj_smoke` / `run_heat_smoke` use tier-2 oracle checksums (not scalar stub / constant 1.0); `li_sim_scientific_version` → 7 (WP-SCI-03 Phase 2 CFD/FEA).
