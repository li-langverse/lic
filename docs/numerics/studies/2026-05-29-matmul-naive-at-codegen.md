...
| Axis | Before (dashboard ingest) | After (local bench, this host) |
|------|---------------------------|--------------------------------|
| **Speed** | Li/cpp **1.333×** (dashboard) | **~0.95×** (li 0.0018s / cpp 0.0019s, n=200) |
...
python3 bench.py --tier 1 --only matmul_naive --verify
```

## Related: `matmul_blocked` codegen (partial)

`emit_matmul2d_blocked_ijk`: vector inner loop now uses `llvm.fmuladd` on `<4 x double>` when fast-math is enabled. Local ratio improved **~1.55× → ~1.29×** vs cpp; still above 1.2× cap — follow-up codegen (register tiles / stack layout) deferred.
