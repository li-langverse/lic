...
- [ ] Phase 7d — Execution decorators — **partial ([#22](https://github.com/li-langverse/lic/issues/22), [#387](https://github.com/li-langverse/lic/issues/387)):** `@vectorized` on `for` → `ArraySimdScope`; **7d-b** `@cpu`/`@gpu`/`@parallel`/`@vectorized` MIR proc tags + `check-mir-*-decorator.sh`; **def `@parallel(disjoint=)`** inherits to nested `parallel for` (policy); **open:** proc `@vectorized` → body `ArraySimdScope` ([#1490](https://github.com/li-langverse/lic/pull/1490)), Lean **G-par** proofs — [PH-7d plan](2026-06-07-ph-7d-g-dec-mir-elaboration.md)
