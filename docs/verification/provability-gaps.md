...
| **G-dec** | Execution decorators | Static elaboration; reserved names; no runtime | **Partial** — **closed slice:** 4× `decorator_exploits` `compile_fail`; `@vectorized` on `for` (`vectorized_for_scope_ok.li`); `@cpu`/`@parallel` Host memory-space MIR + `li_parallel_for_i64` codegen (`check-mir-parallel-portable-lowering.sh`); `std.execution.parallel` policy surface | **7d**, **7e** | `contracts_discharge_corpus.sh`, `decorator_exploits/`, `check-mir-parallel-portable-lowering.sh` |
