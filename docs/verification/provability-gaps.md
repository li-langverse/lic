...
| **G-gpu** | Partial | **Closed slice:** `@gpu` MIR telemetry; **Wave 13:** `ml_gpu_device_buffer_pipeline` + `lig_gpu_device_buffer_ready` host contract. **Open (#110):** cross-space buffer read without `@sync_host` / `@sync_device` must be compile error; `MemorySpace` / `ExecutionSpace` enums in `std/execution/memory_spaces.li` (spec-only). Still open: address-space proofs, full LKIR lowering |
