...
- **WP-SCI-03 QM tier-2 registry** — `sim_scientific_oracle_checksum_qm`, `run_qm_tier2_registry` for algo_registry **401–432** (H₂ STO-3G SCF scaffold via `chem_dft_scf_h2_iteration_scaffold`); `run_algo_registry_tier2.li` extended (401 + 418 vs stub 1.001).
...
- **WP-SCI-03 tier-2 QM registry** — `sim_scientific_oracle_checksum_qm` (harmonic-well + FD kinetic positive checksum), `run_qm_tier2_registry` for algo 401–432; `run_algo_registry_tier2.li` asserts non-stub checksum for 418/401.
