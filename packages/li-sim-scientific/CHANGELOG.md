...
- **WP-SCI-03 Phase 2 CFD/FEA/QM tier-2** — `sim_scientific_oracle_checksum_cfd`, `sim_scientific_oracle_checksum_fea`, `sim_scientific_oracle_checksum_qm`; `run_cfd_tier2_registry`, `run_fea_tier2_registry`, `run_qm_tier2_registry` for algo_ids 205–210 / 211–216 / 401–432; `vertical_cfd_lid_driven_cavity`, `vertical_fea_linear_elasticity`, `algo_cfd_simple`, `algo_fea_linear_elasticity`; extended `run_algo_registry_tier2.li`.
...
- `run_heat_tier2_registry` scoped to 201–204; CFD/FEA/QM rows no longer share heat oracle or `run_algo_registry_stub` (checksum 1.001); `li_sim_scientific_version` → 7 (WP-SCI-03 Phase 2).
