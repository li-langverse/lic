...
| **7d-b (partial)** | `@vectorized(lanes=4)` on `def`; `@no_vectorize` | `vectorized_dot_proc_ok.li` → `mir_vectorized_proc=1`; **no** body `ArraySimdScope` from proc tag |
...
| **G-dec register** | Summary table L24 | "no MIR lowering yet" — stale vs partial `@vectorized for` |
