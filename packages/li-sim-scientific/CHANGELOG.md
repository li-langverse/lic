...
- **WP-SCI-05/06 FEA/CFD tier-2 oracles** — `sim_scientific_oracle_checksum_fea`, `sim_scientific_oracle_checksum_cfd`, `run_fea_tier2_registry` (212–216), `run_cfd_tier2_registry` (206–211); `vertical_fea_linear_elasticity` / `vertical_cfd_lid_driven_cavity`; `run_algo_registry_tier2.li` extended for algo 210/215.
...
- `run_md_lj_smoke` / `run_heat_smoke` use tier-2 oracle checksums (not scalar stub / constant 1.0); `li_sim_scientific_version` → 7 (WP-SCI-05/06 FEA/CFD dispatch).
