...
def _runner_plan_sets(runner: dict) -> tuple[str, set[str], set[str]]:
    """Return runner id, normalized pending ids, normalized completed ids (plan truth)."""
    ...
    for raw in runner.get("state", {}).get("completed_ids") or []:
        completed.add(_normalize_plan_todo_id(str(raw), rid))
...
def reconcile_snapshot_completed(snap: dict, gaps_by_id: dict[str, dict]) -> int:
    """Close plan_debt rows when plan todo is completed and not in plan_pending."""
    closed = 0
    for runner in snap.get("runners") or []:
        if not isinstance(runner, dict):
            continue
        ...
        for gap in gaps_by_id.values():
            ...
            if norm in pending:
                continue
            if norm not in completed and _normalize_plan_todo_id(str(raw), rid) not in completed:
                continue
            gap["status"] = "closed"
            ev = gap.setdefault("evidence", [])
            note = f"snapshot {rid} plan completed (not in plan_pending): {norm}"
            if note not in ev:
                ev.append(note)
            closed += 1
    return closed
...
def ingest_verticals_stubs(gaps_by_id: dict[str, dict]) -> int:
    vert = Path(
        os.environ.get(
            ...
            str(BENCHMARKS / "workloads/competitive"),
        )
    ) / "verticals.toml"
    if not vert.is_file():
        vert = Path(
