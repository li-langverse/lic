...
- **WP-SCI-03 Phase 2 — QM/CFD tier-2 registry** — `sim_scientific_oracle_checksum_qm` (`chem_dft_energy_kernel_hartree`), `sim_scientific_oracle_checksum_cfd` (inlined SPH poly6 pair); `run_qm_tier2_registry` (401–432), `run_cfd_tier2_registry` (521–523); `vertical_cfd_lid_driven_cavity()` in `li-sim`; smokes `run_algo_registry_tier2.li` (QM non-stub), `run_algo_registry_cfd_tier2.li`.
...
- `run_md_lj_smoke` / `run_heat_smoke` use tier-2 oracle checksums (not scalar stub / constant 1.0); `li_sim_scientific_version` → 7 (WP-SCI-03 QM/CFD tier-2).
