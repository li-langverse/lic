...
| **7d-b** | Elaboration → `ParallelFor` / `simd` / host placement MIR tags | **Exit:** `check-mir-{cpu,parallel,vectorized,gpu}-decorator.sh` + `lic verify mir_*_def=` telemetry — **partial:** `@cpu`, `@parallel(disjoint=)`, `@vectorized(lanes=4)`, `@gpu(devices=N)` |
...
| **7d-e** | `decorator def` with **strict naming** (package prefix, typosquat ban), expansion whitelist | **Exit:** `./li-tests/run_all.sh decorator_exploits` all **compile_fail**; wired in `scripts/ci.sh` — **done** |
