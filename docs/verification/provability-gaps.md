...
| **G-par** | Partial | AST `policy_module` rejects missing disjoint, false `disjoint_row`, mut capture, borrow-in-par; **closed slice:** `par_iteration_independent_tile` + `par_memory_disjoint_rows` + `iteration_independent_tile_spec` + `memory_disjoint_rows_spec` (7d-c); full array aliasing Lean proofs open |
