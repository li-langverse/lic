...
| 7e | Math → SIMD MIR | **G-math** | 1d/2d/blocked GEMM codegen done; tier-1 `matmul_blocked` perf open (#463); Done checklist: [2026-06-07-ph7e-simd-matmul-done-criteria.md](2026-06-07-ph7e-simd-matmul-done-criteria.md) | `check-tier1-li-vs-cpp.sh`, `math_linalg/`, `matmul_loop_codegen_witness_gap.sh` |
...
- [ ] Phase 7e — Math → SIMD/parallel lowering — **partial (#27, #148, #463):** **Done slices:** 1d `ArrayDotF64` SIMD (`simd_dot` ≤1.2×), 2D/blocked GEMM codegen (`ArrayMatMul2DF64`, `ArrayMatMulBlocked2DF64` BK=64); **open:** `matmul_blocked` ≤1.2× C++ (#463-B), `matmul_naive` ingest (#463-C); **deferred:** `horner_pure_li` FMA (#11/#9, PH-2f), MIR↔Lean matmul witness (#32); checklist: [2026-06-07-ph7e-simd-matmul-done-criteria.md](2026-06-07-ph7e-simd-matmul-done-criteria.md)
