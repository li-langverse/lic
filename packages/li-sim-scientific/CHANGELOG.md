...
- **WP-SCI-03 Phase 2 CFD/FEA/QM tier-2 oracles** — `sim_scientific_oracle_checksum_cfd` (1D channel lid Jacobi), `sim_scientific_oracle_checksum_fea` (3-node bar truss), `sim_scientific_oracle_checksum_qm` (harmonic well energy drift); `run_cfd_tier2_registry` / `run_fea_tier2_registry` / `run_qm_tier2_registry` dispatch for algo ids 205–210 / 211–216 / 401–432 (418 documented stub); extended `run_algo_registry_tier2.li`.
...
- `run_heat_tier2_registry` narrowed to PDE heat rows 201–204; CFD/FEA rows route to dedicated oracles; `li_sim_scientific_version` → 7 (WP-SCI-03 Phase 2).
