...
| **7d-b** | Elaboration → `ParallelFor` / `simd` / host placement MIR tags | `li-tests/decorators/` positive — **partial:** `@cpu` → `mir_cpu_def=1` (`check-mir-cpu-decorator.sh`); `@vectorized(lanes=4)`, `@no_vectorize`; `@gpu`/`@parallel` MIR tags — **G-dec** |
