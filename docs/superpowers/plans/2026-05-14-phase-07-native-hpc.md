...
| **7d-b** | Elaboration → `ParallelFor` / `simd` / host placement MIR tags | **G-dec** — `check-mir-{parallel,vectorized}-decorator.sh`; `@vectorized(lanes=4)` on `def`, `@no_vectorize` |
| **7d-c** | Structured `disjoint=`; scoped `@vectorized` on `for` (`ArraySimdScope`) | **G-par** — `parallel_disjoint_lean_opaque_gap.sh`, `check-mir-parallel-for-disjoint.sh`; `vectorized_for_scope_ok.li`; iteration-independence Lean specs still open |
...
- [x] **G-dec / G-par** — `@parallel(disjoint=)` / `@vectorized(lanes=4)` on `def` → MIR proc tags (`mir_parallel_disjoint=`, `mir_vectorized_proc=`); wired in `contracts_discharge_corpus.sh` (**#387**)
