...
#### WP-SCI-03 — `run_algo_registry` real kernels (extends existing ID) — **PARTIAL** (2026-06-08)
...
- **Landed:** QM (`chem_dft_gpu_energy_checksum`), CFD (4×4 cavity diffusion), FEA (2-bar stiffness) tier-2 oracles + registry dispatch; `vertical_cfd_lid_driven_cavity` / `vertical_fea_linear_elasticity` in `li-sim`.
- **Remaining:** per-algo kernel depth (401–404 integrals), external LAMMPS/GROMACS column (WP-PLAT-05), 511–523 / 701+ stub rows.
