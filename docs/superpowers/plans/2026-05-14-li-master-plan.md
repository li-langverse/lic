...
| 7d-b–e | Decorator elaborate + exploits | **G-dec** (related: **G-par** on `disjoint=`) | Elaboration MIR proc tags + `decorator_exploits` CI green | language/decorators.md (new), fast-math guide, `check-mir-*-decorator.sh` |
...
- [ ] Phase 7d — Execution decorators — **partial (#22, #150 7d-c):** `@cpu`/`@gpu`/`@parallel`/`@vectorized` MIR proc tags + `check-mir-*-decorator.sh` (**7d-b**); `@vectorized` on `for` → `ArraySimdScope`; **def `@parallel(disjoint=)`** inherits to nested `parallel for` (policy, **G-par** heuristics); **7d-e** `decorator_exploits/` CI green; **open:** Lean **P-dec**, full **G-par** proofs
