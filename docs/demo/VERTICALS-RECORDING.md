...
**Honesty:** `capture_mode=paint_blit` means `studio_shell_paint_fb.c` rasterizes the same region geometry as `studio_paint_shell_chrome` / `layout_studio_shell_adaptive` — not a live `li-studio-demo` window. **Wgpu readback honesty** (product truth in Li): set `LIG_HOST_PRESENT=1` and `LIG_WGPU_READBACK=1` (or `STUDIO_VERTICALS_WGPU_READBACK=1` on record scripts); `studio_vertical_demo_frame` accepts `native_pixel_source` paint_blit (2), wgpu_readback (3), or wgpu_draw_list (4). Proof: `lic check packages/li-render/li-tests/smoke/native_pixels_honesty.li`. Legacy `cpu_chip_only` (grid + topbar chip only) is available via `STUDIO_VERTICALS_CAPTURE_MODE=cpu_chip_only`.
