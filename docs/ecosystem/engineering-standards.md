...
| **Security** | No known CVE class unmitigated for this repo; no trusted creep; config safe-by-default | [Security testing](../testing/security.md); repo exploit/fuzz suites; `nginx_mitigations` / `webserver-bugs` where HTTP |
...
|-----------|-------------------|
| **`lic`** (compiler) | [`security/cve-catalog.json`](../../security/cve-catalog.json), [`cwe-to-li-tests.toml`](../../security/cwe-to-li-tests.toml), `li-tests/run_security.sh`, fuzz nightly |
| **`lis`** (httpd) | [`tier5_http` exploits](https://github.com/li-langverse/lis/tree/main/benchmarks/tier5_http/exploits), `nginx_mitigations.toml`, [webserver-bugs.toml](../../security/webserver-bugs.toml) |
...
| Agent workloads | Prefer concurrent streams, tail latency, proof-friendly bounds — see [li-httpd plan](../superpowers/plans/2026-05-16-li-httpd-plan.md) |
...
**Nginx** remains **oracle + CVE checklist** for httpd — not a code port ([li-httpd plan](../superpowers/plans/2026-05-16-li-httpd-plan.md)).
