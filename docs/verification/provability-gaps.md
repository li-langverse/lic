...
| **G-gpu** | Partial | **Closed slice:** `@gpu` MIR telemetry; **Wave 13:** `ml_gpu_device_buffer_pipeline` + `lig_gpu_device_buffer_ready` host contract. **Open obligation ([#110](https://github.com/li-langverse/lic/issues/110)):** cross-space buffer read without prior `@sync_host` / `@sync_device` must be compile error + open VC until Lean address-space lemmas land. Still open: full LKIR lowering |
