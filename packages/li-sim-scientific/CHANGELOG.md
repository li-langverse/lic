...
- **WP-SCI-03 CFD/FEA/QM tier-2 registry** — `sim_scientific_oracle_checksum_cfd` (SPH poly6 kernel row), `sim_scientific_oracle_checksum_fea` (4-DOF truss Jacobi), `sim_scientific_oracle_checksum_qm_overlap` / `sim_scientific_oracle_checksum_qm_dft`; `run_cfd_tier2_registry` / `run_fea_tier2_registry` / `run_qm_integral_tier2_registry` / `run_qm_dft_tier2_registry` for algo ids 205–210, 211–216, 401–404, 405–432; smokes `run_algo_registry_tier2.li`, `run_algo_registry_qm_tier2.li`; `vertical_fea_linear_elasticity` / `vertical_cfd_lid_driven_cavity` in `li-sim`.
...
- `run_md_lj_smoke` / `run_heat_smoke` use tier-2 oracle checksums (not scalar stub / constant 1.0); `li_sim_scientific_version` → 7 (WP-SCI-03 CFD/FEA/QM dispatch).
- Align echem/oracle smoke version guards with `li_sim_scientific_version` 7.
