...
- **WP-SIM-P2 qm_dft_scf_energy (418)** — `sim_scientific_oracle_checksum_qm_dft_scf`, `run_qm_dft_scf_tier2_registry`; dispatch replaces checksum **1.001** stub; smoke `qm_dft_scf_interface_smoke.li`.
...
- `run_md_lj_smoke` / `run_heat_smoke` use tier-2 oracle checksums (not scalar stub / constant 1.0); `li_sim_scientific_version` → 7 (WP-SIM-P2).
