...
- **WP-SIM-P2 / lic#478 qm_dft_scf_energy (418)** — `sim_scientific_oracle_checksum_qm_dft_scf`, `run_qm_dft_scf_tier2_registry`, `physics.quantum` kernel dispatch; smokes `qm_dft_scf_interface_smoke.li`, composable `import_qm_dft_scf_smoke.li`; `li_sim_summary_v1` QM metrics.
...
- `run_md_lj_smoke` / `run_heat_smoke` use tier-2 oracle checksums (not scalar stub / constant 1.0); `li_sim_scientific_version` → 7 (WP-SIM-P2 / lic#478).
