...
- **WP-SCI-03 Phase 2 CFD/FEA/QM tier-2** — `sim_scientific_oracle_checksum_cfd`, `sim_scientific_oracle_checksum_fea`, `sim_scientific_oracle_checksum_qm_dft`; split PDE registry dispatch (heat 201–204, CFD 205–210, FEA 211–216); real `algo_qm_dft_scf_energy()` (418) via `chem_dft_energy_kernel_hartree`; `vertical_cfd_lid_driven_cavity` / `vertical_fea_linear_elasticity`; smokes extended in `run_algo_registry_tier2.li`, `scientific_oracle_bench.li`.
