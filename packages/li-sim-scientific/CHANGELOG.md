...
- **WP-SCI-03 Phase 2 — CFD/FEA/QM tier-2 registry** — `sim_scientific_oracle_checksum_cfd` (4×4 lid-driven cavity Jacobi), `sim_scientific_oracle_checksum_fea` (2-element truss strain energy), `sim_scientific_oracle_checksum_qm` (`chem_dft_scf_iteration_scaffold`); split heat dispatch to 201–204; `run_cfd_tier2_registry` / `run_fea_tier2_registry` / `run_qm_tier2_registry`; extended `run_algo_registry_tier2.li`.
