...
1. **WP-SCI-03** — `run_algo_registry` real kernels (CFD/FEA/QM rows); extend `run_algo_registry_tier2.li`. **Partial (2026-06-09):** QM 401–432 tier-2 via `run_qm_tier2_registry` + `sim_scientific_oracle_checksum_qm_dft`; CFD/FEA rows still rigid/stub.
