...
| Step | Change | Notes |
|------|--------|-------|
| 1.1 | `runtime/li_rt.c` — `li_warn_occupancy_once()` | Read `_SC_NPROCESSORS_ONLN`; optional `OMPI_COMM_WORLD_SIZE` / `PMI_SIZE`; compare to baked `team_size` from `--cores × --threads-per-core` |
