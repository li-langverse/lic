...
for sample in \
  ...
  test -f "$AUTOVC"
  grep -qE 'Phase 2f: (return expression matches ensures|fixed-bound dot loop witness|P-loop dot4|prelude dot|PH-2i length-1 broadcast)|Li\.Discharge\.(mat2_at2|dot4|broadcast_rhs_len1)' \
    "$AUTOVC" || {
