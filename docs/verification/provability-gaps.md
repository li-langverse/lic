...
| **G-math** | Partial | **Tier-1 perf (open):** six dashboard-red rows per [PH-7e honesty plan](../superpowers/plans/2026-05-30-ph7e-tier1-red-benchmark-honesty.md) — `matmul_blocked` (1.55×), `matmul_naive` (1.33×), `ml_conv2d_forward`, `ml_mlp_forward`, `ml_mlp_train_step`, `num_gmres` (1.4×); `./scripts/check-tier1-li-vs-cpp.sh` advisory (strict with `LI_TIER1_PERF_STRICT=1`). Retracted prior tier-1 closed-slice claim for `matmul_naive` / `horner_pure_li` pending dashboard green ([#424](https://github.com/li-langverse/lic/issues/424)). **Closed slice:** full 2×2 float `@` Lean Prop (`linalg_mat2_at2_float_closed`, `mat2_at2_float_spec`). **Closed slice:** `linalg_dot4_float_closed` (prelude `dot`), `linalg_mat2_callproc_float_closed`, prelude `norm`/`axpy`/**, IKJ `ArrayMatMul2DF64` codegen on `main`. **Closed slice:** prelude `norm`, `axpy`, same-length `**`, scalar×array, `math_linalg/reductions/`, loop-dot witness, P-linalg corpus |
...
| **G-math** | Math / `A @ B` | Shape errors at compile time; no user `simd(...)` | **Partial** — **closed slice:** 9× `prove_lean_ok` linalg + `discharge_linalg_int_lean.sh`; `math_linalg/` compile tests; **tier-1 perf open** (six dashboard-red rows — [PH-7e plan](../superpowers/plans/2026-05-30-ph7e-tier1-red-benchmark-honesty.md)) | **2i**, **7e**, **2f** | `li-tests/math_linalg/`, `li-tests/contracts_verify/linalg_*_closed.li`, `li-tests/tooling/discharge_linalg_int_lean.sh`, `li-tests/tooling/tier1_li_vs_cpp.sh`, `scripts/check-tier1-li-vs-cpp.sh` |
