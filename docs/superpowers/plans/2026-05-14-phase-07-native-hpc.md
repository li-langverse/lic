...
| **7e-b** | Lower `A @ B` for Tier 1 matmul benches | **partial:** `matmul_naive` / `matmul_blocked` pure-Li; **≤1.2× C++** advisory; **deferred:** Lean↔MIR preservation for `ArrayMatMul2DF64` / general SIMD matmul (**G-math**, `mat2_at2_mir_codegen_lean_gap.sh`) |
