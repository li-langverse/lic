...
None — killer gate **PASS** (152 benchmarks dual-mode complete, all sub-gates green). Remaining proof gaps: G-par partial — full dependent-index refinement beyond closed `index_bound_*_spec` slices (elem/row/grid cell done; general dependent subscripts open — see gap register).
