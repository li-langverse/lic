...
- **sim-p2-qm-dft-scf (lic#478)** — `sim_scientific_oracle_checksum_qm_dft_scf`, `run_qm_dft_scf_tier2_registry`, `algo_qm_dft_scf_energy()` (418) dispatch via `echem_dft_h2_energy_hartree()`; smoke `qm_dft_scf_interface_smoke.li`; composable `import_chem_dft_smoke.li`.
...
- `li_sim_scientific_version` → 7 (sim-p2-qm-dft-scf); algo **418** no longer returns checksum **1.001**.
