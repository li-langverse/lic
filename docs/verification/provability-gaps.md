...
| **G-vc** | Partial | **v1 exit gate (Phase 2e):** typed `AutoVC.lean`; `lic verify` → `witnessed_ensures=` + `mir_return_linked=` (`vc_witness.cpp`); `check-autovc-open-goals.sh` green on closed corpus via `contracts_discharge_corpus.sh`. **Closed slice:** call-site `requires`, const-local discharge, E0303/E0304/E0305, `sqrt_open_bound` float `abs` bound (trusted libm axiom). **Still open:** opaque `vec3_dot`-style returns; loop implementations vs closed-form `ensures` |
...
| **G-vc** | VC generation | Contracts → proof obligations | **Partial** — **v1 exit gate:** typed AutoVC + MIR witness telemetry + open-goals checker on closed corpus ([phase-02e plan](../superpowers/plans/2026-05-14-phase-02e-contracts-vc.md)); **closed slice:** call-site `requires`, const-local discharge, E0303/E0304/E0305; open: opaque returns, loop vs closed-form ensures | **2e** | `compiler/verify/vc_witness.cpp`, `vc_emit_contracts.sh`, `mir_vc_witness.sh`, `contracts_discharge_corpus.sh`, `check-autovc-open-goals.sh`, `discharge_caller_requires_lean.sh`, `discharge_caller_requires_local_lean.sh`, `prove_reject/weak_ensures_true.li` |
...
1. **2e** — VC generation (**G-vc**) — v1 exit gate in [phase-02e plan](../superpowers/plans/2026-05-14-phase-02e-contracts-vc.md)  
