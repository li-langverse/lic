...
| **G-vc** | VC generation | Contracts → proof obligations | **Partial** — auto `_proved` + `lic verify witnessed_ensures=` / `mir_return_linked=`; `sqrt_open_bound` float `abs` open; caller-side `requires` | **2e** | `vc_witness.cpp`, `contracts_discharge_corpus.sh`, `mir_vc_witness.sh` |
