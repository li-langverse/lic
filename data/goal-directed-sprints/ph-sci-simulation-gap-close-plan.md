...
| **li-sim-scientific** | partial → tier-2 oracle | CPU + `@gpu` MD oracle smoke | Lib builds with open VC; smokes verify | **Real:** 4-particle LJ chain + 1D heat/CFD/FEA stencil oracles, `run_multi_physics_at_step`. **Placeholder:** `run_algo_registry_stub` (checksum 1.001) for QM 401–432 | WP-SCI-03 QM rows; no LAMMPS/GROMACS external oracle |
...
- **Status:** partial (2026-06-07) — CFD rows 205–210 + FEA rows 211–216 tier-2 oracles landed (`sim_scientific_oracle_checksum_cfd/fea`, `run_cfd_tier2_registry`, `run_fea_tier2_registry`); QM 401–432 remain stub (checksum 1.001).
