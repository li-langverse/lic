# Orchestrator note — performance gap orchestration (`c00ba466`)
...
**Goal-orientation drift:** briefing recommends `ci_maintainer` + `security_auditor`; scorecard adds `gap_explorer` + `plan_verifier`. Neither sources dispatch `bench_improver` for near-threshold numerics. Supervisor should merge recommendation sources before dispatch.
...
4. **Briefing merge:** include scorecard agents in heap dispatch.
...
| `ci_maintainer` | 44 repos missing CI on main | platform |
