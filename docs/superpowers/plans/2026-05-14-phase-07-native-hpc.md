...
| **7d-b** | Elaboration → `ParallelFor` / `simd` / host placement MIR tags | `li-tests/decorators/` positive — **partial:** `@cpu`, `@gpu`, `@parallel`, `@vectorized(lanes=4)`, `@no_vectorize`; `check-mir-*-decorator.sh` |
...
- [x] **7d-b:** MIR proc tags — `check-mir-cpu-decorator.sh`, `check-mir-gpu-decorator.sh`, `check-mir-parallel-decorator.sh`, `check-mir-vectorized-decorator.sh` (wired in `contracts_discharge_corpus.sh`)
- [x] **7d-e:** `decorator_exploits/` CI green (reserved names, typosquat, `parallel_requires_disjoint`)
