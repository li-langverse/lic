...
- **WP-SCI-03 QM/CFD/FEA tier-2 registry** — `sim_scientific_oracle_checksum_qm_dft`, `sim_scientific_oracle_checksum_cfd_cavity`, `sim_scientific_oracle_checksum_fea_bar`; `run_qm_tier2_registry` (401–432), `run_cfd_tier2_registry` (501–505), `run_fea_tier2_registry` (601–610); `vertical_cfd_lid_driven_cavity` / `vertical_fea_linear_elasticity` in `li-sim`; extended `run_algo_registry_tier2.li`.
...
- `run_md_lj_smoke` / `run_heat_smoke` use tier-2 oracle checksums (not scalar stub / constant 1.0); `li_sim_scientific_version` → 7 (WP-SCI-03 QM/CFD/FEA).
