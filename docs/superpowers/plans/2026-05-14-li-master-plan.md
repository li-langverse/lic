...
| 2e | Contracts + refinements | [2026-05-14-phase-02e-contracts-vc.md](2026-05-14-phase-02e-contracts-vc.md) | **Closed slice:** `check-phase-2e-gates.sh` green (**2e-a…g**); **G-vc** partial — float/opaque ensures open |
...
|-------|----------------------|-----------|-------------------------|------------------------|
| 2e | VC generation | **G-vc** | Closed slice: typed AutoVC + MIR witnesses + open-goal checker on corpus (`check-phase-2e-gates.sh`) | [phase-02e plan](2026-05-14-phase-02e-contracts-vc.md), gaps, proof-corpus-roadmap |
...
- [x] Phase 2e — Contracts + refinements — **closed slice (G-vc):** CI `./scripts/check-phase-2e-gates.sh` green — typed `AutoVC.lean` (`vc_emit_contracts.sh`), `lic verify` MIR linkage (`mir_vc_witness.sh` → `witnessed_ensures=` + `mir_return_linked=`), witness hooks (`compiler/verify/vc_witness.cpp`), closed corpus discharge (`contracts_discharge_corpus.sh` + `check-autovc-open-goals.sh`), **E0303/E0304/E0305** negatives; plan [2026-05-14-phase-02e-contracts-vc.md](2026-05-14-phase-02e-contracts-vc.md); float/opaque ensures still **G-vc** open (Phase **2f**)
