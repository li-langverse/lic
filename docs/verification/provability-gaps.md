...
| **G-par** | Partial | AST `policy_module` rejects missing disjoint, false `disjoint_row`, mut capture, borrow-in-par; **Lean disjoint roadmap** in [proof-corpus-roadmap](proof-corpus-roadmap.md#master-plan-backlog-research-what-to-prove-next) (**P-par**, wave-a-7d) |
| **G-dec** | Partial | **`@vectorized` on `def` → `MirFn.vectorized_lanes`** (wave-a-7d); `@vectorized` on `for` → `ArraySimdScope`; `@parallel` elaboration open |
...
| **G-par** | `parallel for` safety | Proved iteration independence | **Partial** — AST `check_module_policies` + string exploit patterns in `policy.cpp`; **Lean roadmap** [P-par](proof-corpus-roadmap.md#master-plan-backlog-research-what-to-prove-next) | **7b**, **7d-c** | `race_shared_memory`, `decorator_exploits` |
...
| **G-dec** | Execution decorators | Static elaboration; reserved names; no runtime | **Partial** — **`@vectorized` on `def` → `MirFn.vectorized_lanes`**; **7d-c** `@vectorized` on `for` → `ArraySimdScope` (#150); `@parallel` elaboration open | **7d** | `decorator_exploits/`, `decorators/`, `vectorized_def_mir_smoke.sh` |
