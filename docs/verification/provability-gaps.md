...
| **G-gpu** | Partial | **Closed slice:** `@gpu` MIR telemetry; **Wave 13:** `ml_gpu_device_buffer_pipeline` + `lig_gpu_device_buffer_ready` host contract. **Open (lic#110):** cross-space sync obligations (`@sync_host` / `@sync_device` before host↔device read); address-space separation proofs; full LKIR lowering — [kokkos rubric](../hpc/kokkos-memory-execution-spaces-rubric.md) |
...
| **G-gpu** | `@gpu` / device buffers | Separate address space proofs + explicit sync | **Partial** — MIR telemetry + **Wave 13** device-buffer host contract (`ml_gpu_device_buffer.li`); **open (lic#110):** `@sync_host` / `@sync_device` compile obligations; vendor codegen slice via `lig_emit_vendor_lowering_ready`; address-space proofs open | **3+**, **7d** | `li-tests/decorators/gpu_*`, `packages/li-ml/li-tests/smoke/ml_gpu_device_buffer.li`, `scripts/check-mir-gpu-decorator.sh`, `docs/hpc/kokkos-memory-execution-spaces-rubric.md` |
