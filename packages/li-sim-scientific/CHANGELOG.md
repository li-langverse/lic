...
- **WP-SCI-03 Phase 2 tier-2 QM/CFD/FEA** — `sim_scientific_oracle_checksum_qm_dft`, `sim_scientific_oracle_checksum_cfd`, `sim_scientific_oracle_checksum_fea`, `cfd_oracle_lid_relax_step`; registry dispatch for PDE 201–204 heat, 205–210 CFD, 211–216 FEA, 401–432 QM; extended `run_algo_registry_tier2.li` (echem rows stay in dedicated echem smokes).
...
- `run_md_lj_smoke` / `run_heat_smoke` use tier-2 oracle checksums (not scalar stub / constant 1.0); `li_sim_scientific_version` → 7 (WP-SCI-03 Phase 2 QM/CFD/FEA).
