...
- **Remaining lib gaps (Phase 2):** `run_algo_registry_stub` for CFD/FEA rows (QM 401–432 tier-2 landed); `li-sim-viz` compose-only; scene `native_pixels` stub; particles MD force accumulation still weak.
...
| **li-sim-scientific** | partial → tier-2 oracle | CPU + `@gpu` MD oracle smoke | Lib builds with open VC; smokes verify | **Real:** 4-particle LJ chain + 1D heat stencil + QM SCF scaffold oracles, `run_multi_physics_at_step`. **Placeholder:** `run_algo_registry_stub` (checksum 1.001) for CFD/FEA registry IDs | WP-SCI-03 partial (QM done); CFD/FEA rows stub; no LAMMPS/GROMACS external oracle |
...
#### WP-SCI-03 — `run_algo_registry` real kernels (extends existing ID) — **partial** (QM 401–432 tier-2)

- **Goal:** Replace `run_algo_registry_stub` for CFD/FEA/QM rows with real dispatch or tier-2 oracles.
- **Landed (2026-06-09):** `sim_scientific_oracle_checksum_qm_dft`, `run_qm_tier2_registry`, smokes `run_algo_registry_qm_tier2.li`; CFD/FEA remain stub.
