#!/usr/bin/env bash
# Run compiler-studio-plan-loop in batches until deadline (default: next Monday 08:00 Europe/Berlin).
set -euo pipefail
...
resolve_deadline_ts() {
  if [[ -n "${COMPILER_STUDIO_UNTIL_ISO:-}" ]]; then
    date -d "$COMPILER_STUDIO_UNTIL_ISO" +%s
    return
  fi
  if [[ "${COMPILER_STUDIO_WEEKEND_MODE:-1}" == "1" ]]; then
    date -d "next monday ${DEADLINE_LOCAL}" +%s
    return
  fi
  deadline_epoch "$DEADLINE_LOCAL"
}

log() { echo "$@" | tee -a "$LOG"; }

DEADLINE_TS="$(resolve_deadline_ts)"

log "==> compiler-studio-plan-until-deadline $(date -Iseconds)"
log "    TZ=${TZ} deadline=$(date -d "@${DEADLINE_TS}" -Iseconds)"
log "    server_utc=$(date -u -Iseconds)"
log "    branch=$COMPILER_STUDIO_PR_BRANCH batch_cap=$BATCH_CAP min_per_iter=${MIN_PER_ITER}m log=$LOG"
