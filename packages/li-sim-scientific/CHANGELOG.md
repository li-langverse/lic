...
- **WP-SCI-03 QM tier-2 registry** — `sim_scientific_oracle_checksum_qm_dft`, `run_qm_tier2_registry` for algo ids 401–432 (`chem_dft_energy_kernel_hartree`); `run_algo_registry_tier2.li` extended (418 + 401 non-stub checksum).
...
- QM registry ids 401–432 dispatch via `run_qm_tier2_registry` (no longer `run_algo_registry_stub` checksum 1.001); `li_sim_scientific_version` → 7.
