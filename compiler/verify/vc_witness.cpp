...
bool expr_is_broadcast_len1_add_ij_eq(const Expr* e, const std::string& c, const std::string& a,
  ...
  const auto a_index_ok = [&](const Expr* side) -> bool {
    return side && side->kind == Expr::Kind::Index && expr_is_ident(side->base.get(), a) &&
           expr_is_int_lit(side->index.get(), idx);
  };
  const auto b_index0_ok = [&](const Expr* side) -> bool {
    return side && side->kind == Expr::Kind::Index && expr_is_ident(side->base.get(), b) &&
           expr_is_int_lit(side->index.get(), 0);
  };
  return (a_index_ok(rhs->lhs.get()) && b_index0_ok(rhs->rhs.get())) ||
         (a_index_ok(rhs->rhs.get()) && b_index0_ok(rhs->lhs.get()));
}
...
bool expr_is_broadcast_len1_add_float4_spec(const Expr& e, const std::string& a,
  ...
  for (std::int64_t idx = 0; idx < 4; ++idx) {
    if (!expr_is_broadcast_len1_add_ij_eq(terms[static_cast<std::size_t>(idx)], c, a, b, idx)) {
      return false;
    }
  }
  return true;
