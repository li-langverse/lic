...
struct EmitCtx {
  ...
  void emit_matmul2d_ijk_loops(llvm::AllocaInst* c_mat, llvm::AllocaInst* a_mat,
    ...
    if (!fp_numerically_stable) {
      ...
      if ((n % 4) == 0) {
        vfma_fn = llvm::Intrinsic::getOrInsertDeclaration(module, llvm::Intrinsic::fmuladd,
                                                          {f64x4});
      }
    }

    const bool vectorize_j = (n % 4) == 0 && vfma_fn != nullptr;
  ...
  void emit_matmul2d_blocked_ijk(llvm::AllocaInst* c_mat, llvm::AllocaInst* a_mat,
    ...
    llvm::Function* vfma_fn = nullptr;
    if (vectorize_j && !fp_numerically_stable) {
      vfma_fn = llvm::Intrinsic::getOrInsertDeclaration(
          module, llvm::Intrinsic::fmuladd, {f64x4});
    }
    auto store_c_vec4 = [&](llvm::Value* i, llvm::Value* k, llvm::Value* j, llvm::Value* aik) {
      ...
      builder->CreateAlignedStore(result, cp, llvm::Align(8));
    };
