...
| **G-vc** | VC generation | Contracts → proof obligations | **Partial** — auto `_proved` for `True`, `decreases`, return/const `ensures`, requires→return mirror (int/float); non-`True` `requires` caller-side; `abs` refinement open | **2e** | `vc_emit_lean.cpp`, `contracts_discharge_corpus.sh`, `discharge_*_lean.sh` |
