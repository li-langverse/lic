...
| **2i-b** | `dot`, `norm`, `axpy`, `**`, reductions | **partial** — prelude `dot`/`norm`/`axpy`; `math_linalg/reductions/` (`sum_non_array.li`, `dot_len_mismatch.li` compile_fail); float Lean Props open |
