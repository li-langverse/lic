...
| **G-vc** | Partial | **Closed slice:** call-site `requires`, const-local discharge, E0303/E0304/E0305; `sqrt_open_bound` float `abs` bound (trusted libm axiom). **2e v1 gate:** [phase-02e exit gates](../superpowers/plans/2026-05-14-phase-02e-contracts-vc.md#exit-gates-ci-must-be-green). Still open: opaque `vec3_dot`-style returns; loop implementations vs closed-form `ensures` |
...
| **G-vc** | VC generation | Contracts → proof obligations | **Partial** — **closed slice:** call-site `requires`, const-local discharge, E0303/E0304/E0305; **2e v1 gate:** [phase-02e](../superpowers/plans/2026-05-14-phase-02e-contracts-vc.md); open: float `abs`, opaque returns | **2e** | `vc_witness.cpp`, `vc_emit_contracts.sh`, `mir_vc_witness.sh`, `discharge_caller_requires_lean.sh`, `discharge_caller_requires_local_lean.sh`, `contracts_discharge_corpus.sh`, `check-autovc-open-goals.sh`, `prove_reject/weak_ensures_true.li` |
