...
| **Decorators (`@parallel`, …)** | Compile-time elaboration + proofs | **Partial** — parse + policy (7d-a/e) + MIR proc tags (`@cpu`, `@gpu`, `@parallel`, `@vectorized`); proc `@vectorized` → `ArraySimdScope`; **open:** 7d-e macro expansion — [PH-7d plan](../superpowers/plans/2026-06-07-ph-7d-g-dec-mir-elaboration.md) ([#22](https://github.com/li-langverse/lic/issues/22)) |
...
| **G-dec** | Partial | **Closed slice (7d-b/c):** `@cpu`/`@gpu`/`@parallel`/`@vectorized` MIR proc tags + `mir_*_def` telemetry + `check-mir-*-decorator.sh`; proc `@vectorized` → `ArraySimdScope` (`mir_vectorized_def_scope`); `decorator_exploits/` CI; Lean **P-dec** open — [plan](../superpowers/plans/2026-06-07-ph-7d-g-dec-mir-elaboration.md) ([#22](https://github.com/li-langverse/lic/issues/22)) |
...
| **G-dec** | Execution decorators | Static elaboration; reserved names; no runtime | **Partial** — **closed slice (7d-b/c):** `@cpu`/`@gpu`/`@parallel`/`@vectorized` MIR proc tags (`mir_cpu_def`, `mir_gpu_def`, `mir_parallel_disjoint`, `mir_vectorized_proc`, `mir_vectorized_def_scope`); `@vectorized` on `for` (`vectorized_for_scope_ok.li`); 4× `decorator_exploits` `compile_fail`; **G-par** disjoint witnesses still policy-only ([#387](https://github.com/li-langverse/lic/issues/387)) | **7d** | `check-mir-*-decorator.sh`, `decorator_exploits/`, [PH-7d plan](../superpowers/plans/2026-06-07-ph-7d-g-dec-mir-elaboration.md) |
...
| Decorators | Never interpreted at run time | **Partial** — proc tags elaborate to MIR; macro expansion open (**7d-e**) |
