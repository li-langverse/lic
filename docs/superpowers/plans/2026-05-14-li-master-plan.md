...
- [ ] Phase 7d — Execution decorators — **partial ([#22](https://github.com/li-langverse/lic/issues/22), [#387](https://github.com/li-langverse/lic/issues/387)):** `@vectorized` on `for` → `ArraySimdScope`; **7d-b** `@cpu`/`@parallel`/`@vectorized` MIR proc tags + `check-mir-*-decorator.sh`; proc `@vectorized` → body `ArraySimdScope` (`mir_vectorized_def_scope`); **def `@parallel(disjoint=)`** inherits to nested `parallel for`; **open:** 7d-e macro expansion, Lean **G-par** proofs — [PH-7d plan](2026-06-07-ph-7d-g-dec-mir-elaboration.md)
