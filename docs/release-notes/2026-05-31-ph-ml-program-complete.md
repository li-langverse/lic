...
**Agent verification (2026-06-06):** run `code_implementer-1780722748465` — rebased #857 onto main; wired remaining PH-ML bench scripts (`async-env-collect`, `lkir-matmul`, `mlp-competitive`) through `li_export_lic` / `lic-bin-select.sh` (native `build/compiler/lic` preferred over non-runnable `build-wsl` on older glibc).
