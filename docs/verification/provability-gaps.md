...
| **Decorators (`@parallel`, …)** | Compile-time elaboration + proofs | **Partial** — MIR proc tags (`@cpu`, `@gpu`, `@parallel`, `@vectorized`); **G-par** disjoint heuristics; Lean **P-dec** open |
...
| **G-dec** | Partial | **Closed slice:** `@cpu`/`@gpu`/`@parallel`/`@vectorized` MIR proc tags + `check-mir-*-decorator.sh` corpus; **G-par** disjoint= still heuristic; Lean **P-dec** open |
...
| **G-dec** | Execution decorators | Static elaboration; reserved names; no runtime | **Partial** — **closed slice:** 4× `decorator_exploits` `compile_fail`; `@vectorized` on `for` (`vectorized_for_scope_ok.li`); `@cpu`/`@gpu`/`@parallel`/`@vectorized` MIR proc tags + `mir_*_def=` telemetry + `check-mir-*-decorator.sh` (**7d-b**); **G-par** `disjoint=` heuristics on `@parallel` (**7d-c**) | **7d** | `contracts_discharge_corpus.sh`, `decorator_exploits/`, `check-mir-cpu-decorator.sh` |
