...
| **Decorators (`@parallel`, …)** | Compile-time elaboration + proofs | **Partial** — parse + policy + MIR proc tags (`@cpu`, `@parallel`, `@vectorized`, `@gpu`); Lean **P-dec** open — see **G-dec**, **G-par** |
...
| **G-dec** | Partial | **Closed slice:** `@cpu` / `@parallel` / `@vectorized` / `@gpu` MIR proc tags + `lic verify` telemetry + `check-mir-*-decorator.sh` corpus; 4× `decorator_exploits` `compile_fail`; Lean **P-dec** open — disjoint witness still **G-par** heuristic |
...
| **G-dec** | Execution decorators | Static elaboration; reserved names; no runtime | **Partial** — **closed slice:** 4× `decorator_exploits` `compile_fail`; `@vectorized` on `for` (`vectorized_for_scope_ok.li`); `@cpu` / `@parallel(disjoint=…)` / `@vectorized(lanes=…)` / `@gpu` MIR proc tags + `check-mir-{cpu,parallel,vectorized,gpu}-decorator.sh` | **7d** | `contracts_discharge_corpus.sh`, `decorator_exploits/`, `li-tests/decorators/` |
