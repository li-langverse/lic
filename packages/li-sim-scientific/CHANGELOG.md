...
- **WP-SCI-03 Phase 2 QM tier-2** — `sim_scientific_oracle_checksum_qm_dft`, `run_qm_tier2_registry` (401–432 dispatch via `chem_dft_scf_iteration_scaffold`); `run_algo_registry_tier2.li` extended for real QM checksum (not stub 1.001).
...
- `run_md_lj_smoke` / `run_heat_smoke` use tier-2 oracle checksums (not scalar stub / constant 1.0); `li_sim_scientific_version` → 7 (WP-SCI-03 Phase 2 QM tier-2).
