...
| Local tier-1 (this pass, `li_rt_volatile_sink_f64`) | **10.0×** | Honest pure-Li scalar codegen vs native C reference |

Evidence rows (`benchmarks/results/latest.csv` on this branch):

```csv
horner_pure_li,cpp,...,wall_time,0.0009,s,...
horner_pure_li,li,...,wall_time,0.0105,s,...
```
...
Still **red** (10.0×). Next work is **PH-7e** codegen (SIMD/unroll Horner), not catalog threshold edits. Lexer `Plus` fix is already on `main` ([autoresearch-horner-lexer-2026-05-18.md](./autoresearch-horner-lexer-2026-05-18.md)).
