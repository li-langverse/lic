...
- **WP-SCI-03 Phase 2 slice** — `sim_scientific_oracle_checksum_fea`, `sim_scientific_oracle_checksum_cfd`, `run_fea_tier2_registry` (211–216), `run_cfd_tier2_registry` (205–210); heat dispatch narrowed to 201–204; `vertical_fea_linear_elasticity` / `vertical_cfd_lid_driven_cavity` + algo ids 214/206 in `li-sim`; tier-2 smoke extended in `run_algo_registry_tier2.li`.
...
- `run_md_lj_smoke` / `run_heat_smoke` use tier-2 oracle checksums (not scalar stub / constant 1.0); `li_sim_scientific_version` → 7 (WP-SCI-03 Phase 2 FEA/CFD slice).
