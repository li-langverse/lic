...
| **G-async** | `@async` / `raises Async` | Structured concurrency proofs | **Partial** — `@async` requires `raises Async`; `@executor(pool=…)` MIR stub + `then` borrow gate (2× `compile_fail`); `Future[T]` / `await` codegen open; HPX futures rubric in [HPX async futures spec](../superpowers/specs/2026-06-07-li-hpx-async-futures-surface.md) (lic#112); cross-link lic#125 `when_all` | **2+**, **7d** | `li-tests/effects/`, `li-tests/decorators/executor_*.li` |
