...
| **G-dec** | Partial | **Closed slice:** MIR proc tags (`mir_parallel_disjoint=`, `mir_vectorized_proc=`) + corpus scripts; Lean **P-dec** open |
...
| **G-dec** | Execution decorators | Static elaboration; reserved names; no runtime | **Partial** — **closed slice:** 4× `decorator_exploits` `compile_fail`; `@vectorized` on `for` (`vectorized_for_scope_ok.li`); `@parallel`/`@vectorized` MIR proc tags on `def` (`mir_parallel_disjoint=`, `mir_vectorized_proc=`) + corpus scripts | **7d** | `contracts_discharge_corpus.sh`, `scripts/check-mir-{parallel,vectorized}-decorator.sh`, `decorator_exploits/` |
