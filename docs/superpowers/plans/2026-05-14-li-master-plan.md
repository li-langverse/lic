...
- [ ] Phase 2i — Math / linalg surface — **partial:** **2i-a/c** (#148); **2i-b** `norm`, `sum`/`dot`, `reductions/`, same-length `**`, prelude `axpy`, scalar×array; **length-1 broadcast** (`array[1]`→`array[N]` element-wise); **`@` shape slice done** (1d dot + 2d GEMM rules, `matmul_2x2_golden.li`, rank-3 reject — [#20](https://github.com/li-langverse/lic/issues/20)); full NumPy rank broadcast still open; float `@` Props closed (`mat2_at2_eval`); SIMD matmul perf → **7e** ([#27](https://github.com/li-langverse/lic/issues/27))
