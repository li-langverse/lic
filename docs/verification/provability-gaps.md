...
| **Decorators (`@parallel`, …)** | Compile-time elaboration + proofs | **Partial** — parse + policy (7d-a/e); `@vectorized for` scoped (#150); **7d-b–e MIR elaboration open** — [plan](../superpowers/plans/2026-06-07-ph-7d-g-dec-mir-elaboration.md) ([#22](https://github.com/li-langverse/lic/issues/22)) |
...
| **G-dec** | Execution decorators | Static elaboration; reserved names; no runtime | **Partial** — **closed slice:** 4× `decorator_exploits` `compile_fail`; `@vectorized` on `for` (`vectorized_for_scope_ok.li`); proc MIR telemetry only — **elaboration open** ([plan](../superpowers/plans/2026-06-07-ph-7d-g-dec-mir-elaboration.md), **#387** proc parity) | **7d** | `contracts_discharge_corpus.sh`, `decorator_exploits/`, `check-mir-*-decorator.sh` |
