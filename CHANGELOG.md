...
- **lic check resource flags:** restore `resource_options_invalid()` so negative `--threads` / `--cores` / `--threads-per-core` fail after `finalize_resource_options` (execution_exploits).
