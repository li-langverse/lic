# Orchestrator note — performance gap orchestration (`3c448d76`)
...
| Gap prep | **Blocked** — `swarm-gap-ingest.py` fails (PyYAML); last apply @ `00:05:46Z` |
...
**Stale registry rows:** `gap-benchmark-red-matmul-naive-tier1`, `gap-benchmark-red-num-gmres-tier1`, and 6+ sibling tier-1 reds remain `open` while `ecosystem-audit.benchmarks.red` is `[]`. Auto-close in ingest when audit green persists.

**httpd perf plan debt:** `gap-phase2-perf-wrk-soak`, `gap-phase2-streaming-wrk` pending on `httpd` runner (snapshot stale @ 2026-05-30). Route via `server_platform` research goal — not a new lic systemd loop.
...
**`orch-r3` reconcile:** `gap-plan-pending-swarm-observer-orch-r3-missing-package-sweep` — route `gap-line-profiler-001` via `issue_planner`; sweep std/HPC package gaps on next `gap_explorer` cadence.

**`orch-r4` reconcile:** `gap-plan-pending-swarm-observer-orch-r4-ui-ux-signals` — route via `ui_ux_quality` goal → `gui_ux_tester`; do not invent new agent ids.

**Skips:** 2 studio-ui patches — `lic-studio-ui` plan path not mounted.

**Goal-orientation drift:** briefing recommends `ci_maintainer` + `security_auditor`; scorecard adds `gap_explorer` + `plan_verifier`. Neither dispatches `bench_improver` for near-threshold numerics. Supervisor should merge recommendation sources before dispatch.
...
5. **Apply script:** wire `orch-r3`/`orch-r4` backlog mappings (currently deferred).
...
| `ci_maintainer` | 33 repos missing CI on main | platform |
...
- `/app/data/runs/swarm_observer-1781193991102.md`
