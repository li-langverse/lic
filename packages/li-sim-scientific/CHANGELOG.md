...
- **WP-SCI-03 Phase 2 tier-2 kernels** — `sim_scientific_oracle_checksum_cfd`, `sim_scientific_oracle_checksum_fea`, `sim_scientific_oracle_checksum_qm_dft`; `run_cfd_tier2_registry` (206–211), `run_fea_tier2_registry` (212–216), `run_qm_tier2_registry` (401–432); `vertical_cfd_lid_driven_cavity` / `vertical_fea_linear_elasticity` in `li-sim`; smokes `run_algo_registry_sci_phase2.li`, updated `run_algo_registry_tier2.li` (QM non-stub).
...
- `run_md_lj_smoke` / `run_heat_smoke` use tier-2 oracle checksums (not scalar stub / constant 1.0); `li_sim_scientific_version` → 7 (WP-SCI-03 Phase 2 CFD/FEA/QM dispatch).
