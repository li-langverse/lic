...
if [[ "${PH_ML_PROGRAM_COMPLETE_INNER:-0}" != "1" ]] && ! li_pick_lic_bin "$ROOT" >/dev/null 2>&1 && command -v wsl.exe >/dev/null 2>&1; then
  wsl_root="$(wsl.exe wslpath -u "$ROOT" 2>/dev/null | tr -d '\r\n')"
  if [[ -n "$wsl_root" ]] && wsl.exe bash -lc "test -x '$wsl_root/build-wsl/compiler/lic/lic' && '$wsl_root/build-wsl/compiler/lic/lic' --version" 2>/dev/null; then
    run_in_wsl
    exit $?
  fi
fi

li_ensure_lic "$ROOT" "ph-ml-program-complete-gates: build lic (./scripts/build.sh or --build-dir build-wsl in WSL)" || exit 1
