...
1. Scorecard: `ecosystem-quality-report.json` refreshed @ 04:45Z → grade C (76.1), `unattended_safe: true`
2. Briefing: `agent-briefing.json` @ 04:44Z — recommends `ci_maintainer`, `security_auditor`; heap schedules `ci_maintainer` only
3. CWE feed: `security-cwe-feed.json` @ 04:42Z vs `lic/security/cve-catalog.json`
4. Gap registry: 62 open rows; security-research plan_debt subset
5. Dry-run: `swarm-gap-ingest.py` (PyYAML failure); apply artifact @ 00:05Z (~28h stale)
...
6. Map ph-db `wp-n5-security-bench` to a runner backlog for apply-actions.
