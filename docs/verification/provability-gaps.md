...
| **G-dec** | Execution decorators | Static elaboration; reserved names; no runtime | **Partial+** — **closed slice:** 4× `decorator_exploits` `compile_fail`; `@vectorized` on `for` (`vectorized_for_scope_ok.li`); tier-2 MD `def` stack (`md_lj_decorator_def_ok.li`, `benchmarks/.../md_lennard_jones/li/kernel.li`); MIR proc tags + corpus scripts | **7d** | `contracts_discharge_corpus.sh`, `decorator_exploits/`, `md_lj_decorator_def_ok.li` |
