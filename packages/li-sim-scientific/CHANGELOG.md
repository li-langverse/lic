...
- **WP-SCI-03 CFD/FEA tier-2 oracles (Phase 2)** — `sim_scientific_oracle_checksum_cfd` (4×4 lid-driven cavity Jacobi), `sim_scientific_oracle_checksum_fea` (3-bar stiffness assembly + GS solve); split `run_algo_registry` dispatch for PDE (201–204), CFD (205–210), FEA (211–216); `vertical_cfd_lid_driven_cavity`, `vertical_fea_linear_elasticity`; extended `run_algo_registry_tier2.li`.
...
- `run_md_lj_smoke` / `run_heat_smoke` use tier-2 oracle checksums (not scalar stub / constant 1.0); `li_sim_scientific_version` → 7 (WP-SCI-03 CFD/FEA split).
