...
def ingest_verticals_stubs(gaps_by_id: dict[str, dict]) -> int:
    default_competitive = str(LANGVERSE / "benchmarks" / "competitive")
    competitive = os.environ.get("BENCHMARKS_COMPETITIVE", default_competitive)
    vert = Path(competitive) / "verticals.toml"
    if not vert.is_file():
        vert = ROOT / "benchmarks/competitive/verticals.toml"
    if not vert.is_file():
        vert = BENCHMARKS / "benchmarks/workloads/competitive/verticals.toml"
    if not vert.is_file():
        return 0
