...
- **WP-SCI-03 FEA/CFD tier-2 registry** — `sim_scientific_oracle_checksum_fea`, `cfd_lid_draught_witness`, `run_fea_tier2_registry` (511–514), `run_cfd_tier2_registry` (521–523); `vertical_fea_linear_elasticity` / `vertical_cfd_lid_driven_cavity`; smokes `fea_linear_elasticity_smoke.li`, `cfd_lid_driven_cavity_smoke.li`; extended `run_algo_registry_tier2.li`.
...
- `run_md_lj_smoke` / `run_heat_smoke` use tier-2 oracle checksums (not scalar stub / constant 1.0); `li_sim_scientific_version` → 7 (WP-SCI-03 FEA/CFD).
