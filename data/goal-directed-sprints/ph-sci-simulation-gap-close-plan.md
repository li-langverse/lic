...
- **Remaining lib gaps (Phase 2):** `run_algo_registry_stub` for QM 405–432 + external MD; `li-sim-viz` compose-only; scene `native_pixels` stub; particles MD force accumulation still weak.
...
- **Landed slice:** CFD 205–210 (`sim_scientific_oracle_checksum_cfd`), FEA 211–216 (`_fea`), QM integrals 401–404 (`_qm_overlap`); 405–432 + external MD still stub.
