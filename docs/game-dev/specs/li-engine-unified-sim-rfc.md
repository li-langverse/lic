...
**SIM-3 (partial):** `packages/li-ml-rl` — `EnvPoolStub`, `env_pool_step` → `sim_step`; `sim_rl` studio route.
...
SIM-0 (this RFC) → SIM-1 step/reset → SIM-2 replay → SIM-3 RL hookup (partial).
