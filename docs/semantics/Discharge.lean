...
/-- Length-1 rhs broadcast add on float4 (matches `linalg_broadcast_len1_add_float4_closed.li`). -/
def broadcast_rhs_len1_add_float4_spec (a : LiArray Float 4) (b : LiArray Float 1)
...
/-- Semantic length-1 rhs broadcast add (reuses index 0 of shorter operand). -/
def broadcast_rhs_len1_add_float4_eval (a : LiArray Float 4) (b : LiArray Float 1) :
    LiArray Float 4 :=
  fun i =>
    match i with
    | ⟨0, _⟩ => (a[0]!) + (b[0]!)
...
theorem broadcast_rhs_len1_add_float4_spec_proved (a : LiArray Float 4) (b : LiArray Float 1) :
    broadcast_rhs_len1_add_float4_spec a b (broadcast_rhs_len1_add_float4_eval a b) := by
  unfold broadcast_rhs_len1_add_float4_spec broadcast_rhs_len1_add_float4_eval
  refine And.intro rfl (And.intro rfl (And.intro rfl rfl))
