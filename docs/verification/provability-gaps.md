...
| **G-par** | Partial | AST `policy_module` rejects missing disjoint, false `disjoint_row`, mut capture, borrow-in-par; **closed slice:** `par_iteration_independent_tile` + `iteration_independent_tile_spec` (7d-c); full memory-disjoint Lean proofs open |
