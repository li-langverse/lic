...
| **G-math** | Partial | **Closed slice (tier-1 codegen):** `ArrayDotF64`/`ArrayBinOpF64` gather SIMD; `ArrayMatMul2DF64` IKJ+FMA (`matmul_loop_codegen_witness_gap.sh`); `ArrayMatMulBlocked2DF64` blocked IKJ (BK=64). **Open (tier-1 perf):** `matmul_naive`, `matmul_blocked`, `horner_pure_li` may exceed 1.2× C++ — `check-tier1-li-vs-cpp.sh` advisory; strict only with `LI_TIER1_PERF_STRICT=1`. **Closed slice:** full 2×2 float `@` Lean Prop (`linalg_mat2_at2_float_closed`, `mat2_at2_float_spec`). **Closed slice:** `linalg_dot4_float_closed` (prelude `dot`), `linalg_mat2_callproc_float_closed`, prelude `norm`/`axpy`/**, `math_linalg/reductions/`, loop-dot witness, P-linalg corpus. **Done criteria:** [phase-07 §7e](../superpowers/plans/2026-05-14-phase-07-native-hpc.md#phase-7e-done-criteria--simd--blocked-gemm-ph-7e--g-math) |
...
| **G-math** | Math / `A @ B` | Shape errors at compile time; no user `simd(...)` | **Partial** — **closed slice:** 9× `prove_lean_ok` linalg + `discharge_linalg_int_lean.sh`; `math_linalg/` compile tests; **7e SIMD matmul:** `ArrayDotF64`/`ArrayBinOpF64` gather SIMD; `ArrayMatMul2DF64` IKJ+FMA (`matmul_loop_codegen_witness_gap.sh`); `ArrayMatMulBlocked2DF64` blocked IKJ (BK=64) — tier-1 `matmul_*`/`horner_pure_li` ratios still advisory gaps | **2i**, **7e**, **2f** | `li-tests/math_linalg/`, `li-tests/contracts_verify/linalg_*_closed.li`, `li-tests/tooling/discharge_linalg_int_lean.sh`, `li-tests/tooling/matmul_loop_codegen_witness_gap.sh`, `li-tests/tooling/horner_fma_numerically_stable_gap.sh`, `scripts/check-tier1-li-vs-cpp.sh` (`simd_dot`, `matmul_naive`, `matmul_blocked`, `horner_pure_li`); Done checklist: [phase-07 §7e Done criteria](../superpowers/plans/2026-05-14-phase-07-native-hpc.md#phase-7e-done-criteria--simd--blocked-gemm-ph-7e--g-math) |
