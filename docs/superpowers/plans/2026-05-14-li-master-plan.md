...
- [ ] Phase 7e — Math → SIMD/parallel lowering — **partial (#148, #150, #155, #463):** loop matmul + FMA horner; tier-1 dashboard green for six formerly-red ids (ingest 2026-06-01); **`check-tier1-li-vs-cpp.sh`** advisory (strict via `LI_TIER1_PERF_STRICT=1`); **open:** full float Lean Props, re-verify perf after codegen
