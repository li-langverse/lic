...
| **Decorators (`@parallel`, …)** | Compile-time elaboration + proofs | **Partial** — parse + policy (7d-a/e); MIR proc tags for `@cpu`/`@parallel`/`@vectorized`/`@gpu` (7d-b); Lean **P-dec** open |
...
| **G-dec** | Partial | **Closed slice:** `@cpu`/`@parallel`/`@vectorized`/`@gpu` MIR proc tags + `mir_*_def` telemetry + `check-mir-*-decorator.sh` corpus; `decorator_exploits/` CI green (7d-e); Lean **P-dec** open |
...
| **G-dec** | Execution decorators | Static elaboration; reserved names; no runtime | **Partial** — **closed slice:** 4× `decorator_exploits` `compile_fail`; `@vectorized` on `for` (`vectorized_for_scope_ok.li`); `@cpu`/`@parallel`/`@vectorized`/`@gpu` MIR proc tags + `mir_cpu_def`/`mir_parallel_disjoint`/`mir_vectorized_proc`/`mir_gpu_def` telemetry; `check-mir-*-decorator.sh` in `contracts_discharge_corpus.sh` | **7d** | `contracts_discharge_corpus.sh`, `decorator_exploits/`, `scripts/check-mir-{cpu,parallel,vectorized,gpu}-decorator.sh` |
...
| Decorators | Never interpreted at run time | **Partial** — MIR proc tags (`MirDecorator`); full LLVM/LKIR lowering open (**G-gpu**, **P-dec**) |
...
| `li-tests/decorator_exploits/` | **Closed slice** — reserved names, typosquat, `parallel_requires_disjoint` (7d-e); see **G-dec** |
