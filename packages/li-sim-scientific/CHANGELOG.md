...
- **WP-SCI-03 Phase 2 slice** — `sim_scientific_oracle_checksum_cfd` (SPH poly6 row), `sim_scientific_oracle_checksum_fea` (bar stiffness), `sim_scientific_oracle_checksum_qm_overlap`; `run_cfd_tier2_registry` (205–210), `run_fea_tier2_registry` (211–216), `run_qm_integral_tier2_registry` (401–404; 418+ stub); extended `run_algo_registry_tier2.li`.
...
- `run_md_lj_smoke` / `run_heat_smoke` use tier-2 oracle checksums (not scalar stub / constant 1.0); `li_sim_scientific_version` → 7 (WP-SCI-03 CFD/FEA/QM tier-2 dispatch).
