...
| **7d-b** | Elaboration → `ParallelFor` / `simd` / host placement MIR tags | `check-mir-{cpu,parallel,vectorized,gpu}-decorator.sh` + `li-tests/decorators/` — **partial:** proc tags landed; `@async` / `decorator def` expansion open |
| **7d-c** | Structured `disjoint=`; scoped `@vectorized` on `for` (`ArraySimdScope`) | `vectorized_for_scope_ok.li`, `parallel_with_disjoint.li` — **partial:** disjoint witness still **G-par** heuristic |
| **7d-d** | `std/execution/decorators.li` + `docs/language/decorators.md` | Handbook + gallery |
| **7d-e** | `decorator def` with **strict naming** (package prefix, typosquat ban), expansion whitelist | `./li-tests/run_all.sh decorator_exploits` all **fail** except control; CI on every PR |
