...
| Gap | Status |
|-----|--------|
| G-par | **Done** — all **compiler-supported** disjoint surface closed: `disjoint_elem/row/slice/lookup/mod` specs + dependent-index slices (elem/row/grid, affine, blocked-affine, lookup, mod, reverse/rotate/lookup_const permutation) in `Discharge.lean` + proofs gate smokes; org-wide general subscripts beyond compiler surface tracked separately |
