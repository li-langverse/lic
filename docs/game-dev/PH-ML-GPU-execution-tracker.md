...
| WP-HW-07 | **blocked** | feat/ph-ml-gpu-swarm | SPIR-V validate → `bid=5` status `0` | Vulkan compute + [lavapipe CI](../ci/lavapipe-vulkan-smoke.md) |
| WP-HW-08 | **blocked** | feat/ph-ml-gpu-swarm | CPU 2×2 ref + `kernel_matmul_parity.li` | Real GPU device dispatch |
| WP-HW-09 | **blocked** | feat/ph-ml-gpu-swarm | `LIG_EMIT_CUDA=1`; bench `cuda_hardware` | PTX/nvcc — `CUDA_HOME` unset on RTX 3060 lab |
...
| WP-GPU-04 | partial | feat/ph-ml-gpu-swarm | `gpu_decorator_mir.li` (2× `@gpu`), `mir_gpu_proc` | G-gpu proofs |
| WP-GPU-05 | partial | feat/ph-ml-gpu-swarm | 2× compile_fail + Lean note | `P-gpu-*` corpus |
...
| WP-BENCH-ML-05 | done | feat/ph-ml-gpu-swarm | `bench-lig-gpu-suite.sh` wave 4c JSON | `gpu_timing_ns` N/A |
...
| WP-RL-04 | partial | feat/ph-ml-gpu-swarm | `async_env_pool_serial.li` in-process **done** | Fork IPC / multi-process |
...
- [x] Waves 0–5 + 4b/4c tracker rows updated
