...
| **G-vc** | VC generation | Contracts → proof obligations | **Partial** — auto `_proved` for `True`, `decreases`, return/const/mirror `ensures`; `sqrt_open_bound` keeps float `abs` open; non-`True` `requires` caller-side | **2e** | `vc_emit_lean.cpp`, `contracts_discharge_corpus.sh`, `discharge_sqrt_contract_lean.sh` |
