...
| **Decorators (`@parallel`, …)** | Compile-time elaboration + proofs | **Partial** — parse + policy (7d-a/e) + MIR proc tags (`@cpu`, `@gpu`, `@parallel`, `@vectorized`); full elaboration open — [PH-7d plan](../superpowers/plans/2026-06-07-ph-7d-g-dec-mir-elaboration.md) |
...
| **G-dec** | Partial | **Closed slice:** `@cpu`/`@gpu`/`@parallel`/`@vectorized` MIR proc tags + `mir_*_def` telemetry + `check-mir-*-decorator.sh`; `decorator_exploits/` CI; Lean **P-dec** open — [#22](https://github.com/li-langverse/lic/issues/22) |
...
| **G-dec** | Execution decorators | Static elaboration; reserved names; no runtime | **Partial** — **closed slice:** 4× `decorator_exploits` `compile_fail`; `@vectorized` on `for` (`vectorized_for_scope_ok.li`); `@cpu`/`@gpu`/`@parallel`/`@vectorized` MIR proc tags (`mir_cpu_def`, `mir_gpu_def`, `mir_parallel_disjoint`, `mir_vectorized_proc`); `check-mir-*-decorator.sh` — see [PH-7d plan](../superpowers/plans/2026-06-07-ph-7d-g-dec-mir-elaboration.md); **G-par** disjoint witnesses still policy-only ([#387](https://github.com/li-langverse/lic/issues/387)) | **7d** | `contracts_discharge_corpus.sh`, `decorator_exploits/`, `check-mir-cpu-decorator.sh` |
