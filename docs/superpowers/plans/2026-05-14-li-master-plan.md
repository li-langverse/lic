...
- [ ] Phase 2i — Math / linalg surface — **partial:** 1d `float` `@`, `sum(array)`, 2d `array[M,array[K,float]] @` (**2i-c**); SIMD matmul (**7e**) deferred
...
- [ ] Phase 7e — Math → SIMD/parallel lowering — **partial:** `dot`/`@` + `simd_dot` pure-Li (**7e-a** partial); `matmul_naive` pure-Li (**7e-b** partial); explicit SIMD MIR + `@vectorized` deferred
