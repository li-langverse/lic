...
- Implementing `hostbuffer` / `devicebuffer` codegen or LKIR lowering (**blocked** until this plan is **`plan-approved`** and #128 layout ABI is frozen).
...
| OpenMP vs Threads pool conflict | **Policy:** default `OpenMP` execution space; `Threads` opt-in with runtime warning if libomp also linked | Document in `docs/hpc/kokkos-memory-execution-spaces-rubric.md` |

## Sub-phases

| Sub | Deliverable | Exit gate |
|-----|-------------|-----------|
| **A** | **Policy matrix doc** — `docs/hpc/kokkos-memory-execution-spaces-rubric.md` with tables above + Kokkos 4.6.02 notes | Maintainer review; linked from #110 |
| **B** | **Spec enums** — extend [language design §Phase 3](../specs/2026-05-14-li-language-design.md) with `MemorySpace`, `ExecutionSpace`, `View[T, Space]` surface (spec-only) | Spec PR; no parser |
| **C** | **Copy/sync contract** — table: decorator stack → required sync points; cross-link #128 layout types | Plan-only; feeds #15 elaboration |
| **D** | **shared_c_kernel → explicit copy migration rubric** — document transition for tier-2 rows: C oracle owns device data today → pure-Li must name sync | Annotate `heat_equation_2d`, `md_lennard_jones` in migration appendix |
| **E** | **Pilot row plan** — `heat_equation_2d` staged path: (1) host-only pure-Li with `@cpu` `@parallel`, (2) add `hostbuffer`/`devicebuffer` pair + explicit `@sync_device` before kernel, (3) drop `LI_EXTRA_C` | Checklist on #110; implementation **after** `plan-approved` + #128 ABI |
| **F** | **Vendor policy handoff** — file benchmarks PR checklist for Kokkos **4.6.x** pin ([#27](https://github.com/li-langverse/benchmarks/issues/27)); lic does not own catalog | Linked issue on benchmarks; no threshold changes |
| **G** | **G-par / G-gpu gap update** — [provability-gaps.md](../../verification/provability-gaps.md): add “cross-space access without sync” open obligation | Honest Partial; no Done claim |
...
**Exit (plan phase):** rubric + spec enums merged; migration appendix + benchmarks vendor checklist filed.

**Exit (implementation phase, post-approval):** one tier-2 row with explicit sync in source, `li_pure=True`, green checksum, no silent copy in MIR dump.
...
| Gap | Move | Notes |
|-----|------|-------|
| **G-par** | Partial → documented Partial | Execution-space + disjoint rules; Lean proofs remain open |
| **G-gpu** | Partial → documented Partial | Cross-space sync obligations; no new trusted axioms in plan phase |
| **G-dec** | Partial | `@cpu` / `@gpu` must elaborate to sync MIR tags (#15) |
| **G-math** | No change in plan phase | Memory model orthogonal to tier-1 matmul |

## Rollout

1. Merge **this plan** on **`plan-approved`** (lic PR from #110).
2. Sub-phase **A–C** — docs-only PRs on **lic** (can stack after approval).
3. Sub-phase **D–E** — migration appendix + pilot checklist; coordinate with #128 before stride/layout names freeze.
4. Sub-phase **F** — **benchmarks** PR for Kokkos 4.6.x vendor pin (human review; no agent self-merge).
5. Sub-phase **G** — provability-gaps update in same PR as spec enums.
6. Close #110 when rubric + spec + migration plan + vendor handoff are merged; track codegen in #15 / #116.
...
- [ ] Label **`plan-approved`** on #110 after reviewing this doc (remove **`plan-needed`**).
- [ ] Approve final `MemorySpace` / `ExecutionSpace` / sync intrinsic names before parser work.
- [ ] Approve **benchmarks** Kokkos 4.6.x vendor policy PR ([#27](https://github.com/li-langverse/benchmarks/issues/27)).
- [ ] Any `trusted.lean` axiom for device memory — separate human-approved issue.
- [ ] Resolve **Threads vs OpenMP** default with HPC maintainer before runtime ships.
