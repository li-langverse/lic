...
| **G-gpu** | Partial | **Closed slice:** `@gpu` MIR telemetry; **Wave 13:** `ml_gpu_device_buffer_pipeline` + `lig_gpu_device_buffer_ready` host contract. **Open (#110):** cross-space buffer read without `@sync_host` / `@sync_device`; address-space Lean proofs; full LKIR lowering |
...
| **G-gpu** | `@gpu` / device buffers | Separate address space proofs | **Partial** — MIR telemetry + **Wave 13** device-buffer host contract (`ml_gpu_device_buffer.li`); vendor codegen slice via `lig_emit_vendor_lowering_ready`; **open (#110):** cross-space access without explicit sync must be compile error + Lean discharge; address-space proofs open | **3+**, **7d** | `li-tests/decorators/gpu_*`, `packages/li-ml/li-tests/smoke/ml_gpu_device_buffer.li`, `scripts/check-mir-gpu-decorator.sh`, `docs/hpc/kokkos-memory-execution-spaces-rubric.md`, `li-tests/hpc/memory_spaces_spec_smoke.li` |
