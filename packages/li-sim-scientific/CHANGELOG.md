...
- **WP-SCI-03 CFD/FEA/QM tier-2** — `sim_scientific_oracle_checksum_cfd`, `_fea`, `_qm_overlap`, `_qm_dft`; `run_cfd_tier2_registry` (205–210), `run_fea_tier2_registry` (211–216), `run_qm_integral_tier2_registry` (401–404), `run_qm_dft_tier2_registry` (405–432); `vertical_cfd_lid_driven_cavity`, `vertical_fea_linear_elasticity`; extended `run_algo_registry_tier2.li`.
...
- **WP-SCI-03 echem smoke alignment** — echem interface smokes expect `li_sim_scientific_version` 7; AIMD oracle uses kinetic-PE / negated-DFT fallback when energy drift is exactly zero; `scientific_oracle_bench` version gate → 7.
