...
python3 scripts/validate-httpd-config.py packages/li-net-httpd/examples/agent_gateway_limits.toml
python3 scripts/validate-httpd-config.py li-tests/config_desugar/reject/proxy_without_rate_limit.toml && exit 1 || true
HTTPD_GATES_SKIP_LIC_BUILD=1 HTTPD_SKIP_LI_ROUTING_BIN=1 ./scripts/httpd-plan-gates.sh
```
